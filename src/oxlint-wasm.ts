...
export interface OxlintWasmFile {
  path: string;
  source: string;
  lintPath?: string;
}

export interface OxlintWasmLabel {
  label?: string;
  span: {
    offset: number;
    length: number;
...
export interface OxlintWasmDiagnostic {
  ...
  help?: string;
  filename: string;
  labels: OxlintWasmLabel[];
  related: unknown[];
}

export interface OxlintWasmFileResult {
  path: string;
  status: "ok" | "ignored" | "error";
  diagnostics: OxlintWasmDiagnostic[];
  error: string | null;
}

export interface OxlintWasmResult {
  engine: "friendly-adversary-oxlint-wasm";
  abiVersion: 2;
  upstreamVersion: "1.76.0";
  upstreamCommit: "65fe65d8429e1d1bdf86c517ff08bd119ee87660";
  wasmSha256: string;
  files: OxlintWasmFileResult[];
}

export interface OxlintWasmOptions {
  files: OxlintWasmFile[];
  configJson?: string;
  configPath?: string;
  timeoutMs: number;
  maxFileBytes?: number;
...
function comparePaths(left: OxlintWasmFile, right: OxlintWasmFile): number {
  return left.path < right.path ? -1 : left.path > right.path ? 1 : 0;
}

async function defaultWasmPath(): Promise<string> {
  const candidates = [
...
function validateRawResponse(response: RawResponse): void {
...
export async function runOxlintWasm(options: OxlintWasmOptions): Promise<OxlintWasmResult> {
  const major = Number.parseInt(process.versions.node.split(".")[0] ?? "0", 10);
  if (major < 22) throw new FriendlyAdversaryError(`Oxlint WebAssembly requires Node 22 or newer, received ${process.version}`, 2);
  ...
  if (options.signal?.aborted) throw new FriendlyAdversaryError("Oxlint WebAssembly review was cancelled", 130);

  const files = [...options.files].sort(comparePaths);
  const seen = new Set<string>();
  let totalBytes = 0;
  for (const file of files) {
    if (seen.has(file.path)) throw new FriendlyAdversaryError(`Duplicate Oxlint input path: ${file.path}`, 2);
    seen.add(file.path);
  ...
  const response = await new Promise<WorkerResponse>((resolve, reject) => {
    ...
    worker.once("exit", (code) => {
      if (code !== 0) finish(() => reject(new FriendlyAdversaryError(`Oxlint WebAssembly worker exited with code ${code}`, 2)));
    });
  });
  await worker.terminate();
  if (!response.ok) throw new FriendlyAdversaryError(`Oxlint WebAssembly failed: ${response.error}`, 2);
  ...
  const results = response.responses.map((raw, index): OxlintWasmFileResult => {
    validateRawResponse(raw);
    const file = files[index];
    if (!file) throw new FriendlyAdversaryError("Oxlint WebAssembly response has no matching input", 2);
    return {
      path: file.path,
      status: raw.status,
