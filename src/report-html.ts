...
function inline(value: string): string {
  ...
  return escaped
    ...
    .replace(/\[([^\]]+)\]\(([^)]+)\)/gu, (_match, label: string, target: string) => {
...
export function renderMarkdownReport(markdown: string, title = "Friendly Adversary report"): string {
  if (detectRecognizableSecret(markdown)) throw new FriendlyAdversaryError("FA_HTML_SECRET: report Markdown contains credential-like material", 2);
  const lines = markdown.replaceAll("\r\n", "\n").split("\n");
  const body: string[] = [];
  const navigation: Array<{ level: number; id: string; label: string }> = [];
  const used = new Set<string>();
  let list: "ul" | "ol" | undefined;
  let code = false;
  let codeLanguage = "";
  const closeList = () => { if (list) body.push(`</${list}>`); list = undefined; };
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index] ?? "";
    if (line.startsWith("```")) {
      closeList();
      if (code) body.push("</code></pre>");
      else {
    ...
    if (code) { body.push(`${escapeHtml(line)}\n`); continue; }
    ...
    if (unordered || ordered) {
      ...
      if (list !== next) { closeList(); list = next; body.push(`<${next}>`); }
      body.push(`<li>${inline((unordered ?? ordered)?.[1] ?? "")}</li>`);
      continue;
    }
    closeList();
    if (!line.trim()) continue;
    if (/^>\s?/u.test(line)) body.push(`<blockquote>${inline(line.replace(/^>\s?/u, ""))}</blockquote>`);
    else if (/^---+$/u.test(line.trim())) body.push("<hr>");
    else body.push(`<p>${inline(line)}</p>`);
  }
  closeList();
  if (code) throw new FriendlyAdversaryError("FA_HTML_MARKDOWN_INVALID: report contains an unclosed code fence", 2);
...
export async function writeHtmlCompanion(runDirectory: string, markdownName = "report.md", htmlName = "report.html", title?: string): Promise<void> {
