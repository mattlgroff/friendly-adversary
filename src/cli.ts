...
import type { ReviewHost } from "./authority.js";
...
function parse(argv: string[]): Parsed {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (!token?.startsWith("--")) throw new FriendlyAdversaryError(`Unexpected argument: ${token}`, 2);
    const key = token.slice(2);
    if (values.has(key)) throw new FriendlyAdversaryError(`Duplicate option: --${key}`, 2);
    if (BOOLEAN_FLAGS.has(key)) {
      values.set(key, true);
      continue;
    }
    if (!VALUE_FLAGS.has(key)) throw new FriendlyAdversaryError(`Unknown option: --${key}`, 2);
    const next = rest[index + 1];
    if (next === undefined) throw new FriendlyAdversaryError(`Missing value for --${key}`, 2);
    values.set(key, next);
    index += 1;
  }
...
function value(args: Parsed, key: string): string | undefined {
  const found = args.values.get(key);
  return typeof found === "string" ? found : undefined;
}

async function discoveredLenses(): Promise<string[]> {
...
function lensSelection(raw: string | undefined, defaults: string[]): string[] {
...
async function options(args: Parsed) {
  ...
  const host: ReviewHost = hostValue;
  return {
...
function host(args: Parsed): ReviewHost {
  const found = value(args, "host") ?? "unavailable";
  if (found !== "claude-code" && found !== "codex" && found !== "unavailable") throw new FriendlyAdversaryError("--host must be claude-code, codex, or unavailable", 2);
  return found;
}

async function workflowKind(runDirectory: string): Promise<string | undefined> {
  try {
...
function runPath(args: Parsed): string {
  const found = value(args, "run");
  if (!found) throw new FriendlyAdversaryError("This command requires --run <directory>", 2);
...
async function main(): Promise<void> {
  ...
  if (args.command === "audit") {
    const timeoutMs = Number(value(args, "timeout-ms") ?? DEFAULT_TIMEOUT_MS);
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1_000) throw new FriendlyAdversaryError("--timeout-ms must be an integer of at least 1000", 2);
  ...
  if (args.command === "validate") {
    ...
    for (const line of await validateRepository(root)) process.stdout.write(`${line}\n`);
    return;
  }
