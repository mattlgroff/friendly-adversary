...
import { runOxlintWasm, type OxlintWasmFile } from "./oxlint-wasm.js";
...
interface GeneratedBundleOmission {
  path: string;
  bytes: number;
  sha256: string;
...
async function analyzerBatches(repo: string, files: string[], enabled: boolean): Promise<string[][]> {
...
function batchedToolName(name: string, index: number, count: number): string {
...
async function fileOnPath(name: string, extensions: string[]): Promise<string | undefined> {
  const bases = name.includes(path.sep)
    ? [name]
    : (process.env.PATH ?? "").split(delimiter).filter(Boolean).map((directory) => path.join(directory, name));
...
export function windowsNodeCommandShimTarget(content: string): string | undefined {
...
async function windowsNodeCommandShimInvocation(name: string): Promise<Invocation | undefined> {
  if (process.platform !== "win32") return undefined;
  const shim = await fileOnPath(name, [".CMD"]);
  if (!shim) return undefined;
  const content = await readFile(shim, "utf8").catch(() => "");
  if (!content || Buffer.byteLength(content) > 64 * 1024) return undefined;
  ...
  if (relativeTarget) {
    ...
    if (!resolved || !isInside(path.dirname(path.dirname(shim)), resolved)) return undefined;
...
async function npmInvocation(): Promise<Invocation | undefined> {
  ...
  for (const candidate of candidates) {
    ...
    if (resolved) return { executable: process.execPath, argsPrefix: [resolved] };
  }
  return undefined;
}
...
async function localNodeBinInvocation(repo: string, name: string): Promise<Invocation | undefined> {
  const modules = path.join(repo, "node_modules");
  if (!await pathExists(path.join(modules, ".bin", `${name}.cmd`))) return undefined;
  ...
  for (const entry of await readdir(modules, { withFileTypes: true }).catch(() => [])) {
    ...
    for (const scoped of await readdir(candidate, { withFileTypes: true }).catch(() => [])) {
      if (scoped.isDirectory()) packageDirectories.push(path.join(candidate, scoped.name));
    }
  }
  for (const packageDirectory of packageDirectories) {
    try {
      ...
      if (!isInside(packageDirectory, resolved) || !(await lstat(resolved)).isFile()) continue;
      return { executable: process.execPath, argsPrefix: [resolved] };
    } catch {
...
async function repositoryInvocation(repo: string, name: string): Promise<Invocation | undefined> {
  const repoReal = await realpath(repo).catch(() => path.resolve(repo));
  if (name === "npm") return npmInvocation();
  if (name === "pnpm" || name === "yarn" || name === "bun") {
    const commandShim = await windowsNodeCommandShimInvocation(name);
    if (commandShim) return commandShim;
    const manager = await executableOnPath(name);
    if (manager) return { executable: manager, argsPrefix: [] };
  }
  ...
  if (pythonLocal) return { executable: pythonLocal, argsPrefix: [] };
  if (process.platform === "win32") {
    ...
    if (nodeLocal) return { executable: nodeLocal, argsPrefix: [] };
  }
  return undefined;
}

async function repositoryNodeInvocationFrom(
  repo: string,
  ...
  while (isInside(repoReal, directory)) {
    if (process.platform === "win32") {
      ...
      if (executable) return { executable, argsPrefix: [] };
    }
    if (directory === repoReal) break;
    directory = path.dirname(directory);
  }
  return undefined;
}

async function anyFile(repo: string, files: string[]): Promise<boolean> {
  for (const file of files) if (await pathExists(path.join(repo, file))) return true;
  return false;
}

async function reviewableFiles(repo: string, files: string[], extensions: Set<string>): Promise<string[]> {
  const selected: string[] = [];
  for (const file of files) {
    if (!extensions.has(path.extname(file).toLowerCase())) continue;
    try {
      const metadata = await lstat(path.join(repo, file));
      if (metadata.isFile() && !metadata.isSymbolicLink()) selected.push(file);
    } catch {
...
export async function analyzerReadableRoots(...roots: string[]): Promise<string[]> {
...
async function generatedBundleOmissions(
  repo: string,
  assetsRoot: string,
  mergeBaseSha: string,
  files: string[],
): Promise<GeneratedBundleOmission[]> {
  ...
  if (semgrepCandidates.length === 0 && ruffCandidates.length === 0) return [];
  ...
  const trustedAsset = async (relative: string): Promise<{ content: Buffer; source: string }> => {
    if (!selfReview) {
      return {
  ...
  const omissions: GeneratedBundleOmission[] = [];
  if (semgrepCandidates.length) {
    ...
    const manifest = JSON.parse(trustedSemgrepManifest.content.toString("utf8")) as {
      files?: Array<{ path?: string; bytes?: number; sha256?: string }>;
    };
    const generatedRuntimeFiles = new Map(
      (manifest.files ?? [])
        .filter((entry): entry is { path: string; bytes: number; sha256: string } => (
    ...
    for (const file of semgrepCandidates) {
      ...
      if (!metadata?.isFile() || metadata.isSymbolicLink() || metadata.size !== expected.bytes) continue;
      const content = await readFile(absolute);
      const digest = createHash("sha256").update(content).digest("hex");
      if (digest !== expected.sha256) continue;
      omissions.push({
        path: file,
  ...
  if (ruffCandidates.length) {
    ...
    const ruffLock = JSON.parse(trustedRuffLock.content.toString("utf8")) as {
    ...
    if (!prefix || file.slice(prefix.length) !== "runtime/ruff_wasm.js") continue;
    const absolute = path.join(repo, ...file.split("/"));
    const metadata = await lstat(absolute).catch(() => undefined);
    if (!metadata?.isFile() || metadata.isSymbolicLink() || metadata.size !== trustedRuffContent.byteLength) continue;
    const content = await readFile(absolute);
    const digest = createHash("sha256").update(content).digest("hex");
    if (digest !== expectedRuffSha256) continue;
    omissions.push({
      path: file,
      bytes: content.byteLength,
      sha256: digest,
...
async function nearestTypeScriptConfig(repo: string, file: string): Promise<string | undefined> {
  ...
  while (isInside(repositoryRoot, directory)) {
    ...
    for (const candidate of candidates) {
      ...
      if (await pathExists(config)) return path.relative(repositoryRoot, config).split(path.sep).join("/");
    }
...
async function typescriptConfigs(repo: string, files: string[]): Promise<string[]> {
  const configs = new Set<string>();
  for (const file of files) {
    if (!TYPESCRIPT_EXTENSIONS.has(path.extname(file).toLowerCase())) continue;
...
async function typescriptPlans(input: Input, files: string[]): Promise<Plan[]> {
  ...
  for (const [index, config] of configs.entries()) {
    ...
    const tsc = await repositoryNodeInvocationFrom(
      input.repo,
      path.dirname(path.join(input.repo, config)),
      "tsc",
    );
    if (!tsc) {
      plans.push(skipped(
        name,
        `TypeScript project ${config} was found, but its repository-installed TypeScript compiler is unavailable`,
        true,
      ));
      continue;
    }
...
async function pythonProjectPlans(input: Input, files: string[]): Promise<Plan[]> {
...
function skipped(name: string, reason: string, required = false): Plan {
  return { name, args: [], versionArgs: [], findingExitCodes: [], reason, required };
}

function isJavaScriptTestFile(file: string): boolean {
  return /(?:^|[.-])(?:spec|test)\.[^.]+$/u.test(path.basename(file));
}

function focusedTestArguments(script: string, packageDirectory: string, input: Input): string[] {
  if (!/^\s*(?:bun\s+test|vitest(?:\s+run)?|jest|node\s+--test)\b/u.test(script)) return [];
...
async function repositoryScriptPlans(input: Input): Promise<Plan[]> {
  if (!await pathExists(path.join(input.repo, "package.json"))) return [];
  let rootManifest: { scripts?: Record<string, unknown>; packageManager?: unknown };
  try {
    rootManifest = JSON.parse(await readFile(path.join(input.repo, "package.json"), "utf8")) as {
  ...
  if (await pathExists(path.join(input.repo, "pnpm-lock.yaml"))) lockManagers.add("pnpm");
  if (await pathExists(path.join(input.repo, "yarn.lock"))) lockManagers.add("yarn");
  if (await pathExists(path.join(input.repo, "bun.lock")) || await pathExists(path.join(input.repo, "bun.lockb"))) lockManagers.add("bun");
  if (await pathExists(path.join(input.repo, "package-lock.json")) || await pathExists(path.join(input.repo, "npm-shrinkwrap.json"))) lockManagers.add("npm");
  ...
  if (!invocation) return [skipped("repository-scripts", `${manager} is required by the repository but is not installed`, true)];
  const packageDirectories = new Set<string>([path.resolve(input.repo)]);
  ...
  for (const directory of [...packageDirectories].sort()) {
    ...
    try {
      manifest = directory === path.resolve(input.repo)
        ? rootManifest
        : JSON.parse(await readFile(path.join(directory, "package.json"), "utf8")) as { scripts?: Record<string, unknown> };
    } catch {
    ...
    for (const name of ["lint", "typecheck", "type-check", "test", "build", "validate"] as const) {
...
async function changedSymbols(input: Input, files: string[]): Promise<string[]> {
    ...
    try {
      ...
      for (const pattern of patterns) {
        for (const match of content.matchAll(pattern)) {
          ...
          if (symbols.size >= 100) return [...symbols].sort();
        }
      }
    } catch {
...
async function regularFileDigest(file: string): Promise<string | undefined> {
  try {
...
async function replicaFiles(repo: string, relativeRoot: string, current = relativeRoot): Promise<string[]> {
  const files: string[] = [];
  for (const entry of await readdir(path.join(repo, ...current.split("/")), { withFileTypes: true }).catch(() => [])) {
    const relative = `${current}/${entry.name}`;
    if (entry.isDirectory()) files.push(...await replicaFiles(repo, relativeRoot, relative));
    else if (entry.isFile() && SEARCH_REPLICA_EXTENSIONS.has(path.extname(entry.name).toLowerCase())) files.push(relative);
  }
  return files;
}

function canonicalReplicaPath(relative: string): string | undefined {
  if (/^(?:engines|rules|third-party)\//u.test(relative)) return relative;
  if (relative.startsWith("scripts/runtime/wasm/oxlint/")) return `wasm/oxlint/${relative.slice("scripts/runtime/wasm/oxlint/".length)}`;
  const lens = /^references\/lenses\/(.+)\.md$/u.exec(relative)?.[1];
  if (lens) return `lenses/${lens}/LENS.md`;
  if (relative === "LICENSE" || relative === "LICENSING.md") return relative;
  return undefined;
}

async function generatedReplicaSearchExclusions(repo: string): Promise<string[]> {
  const filesByRoot = new Map<string, Map<string, string>>();
  ...
  for (const relative of allRelative) {
    const replicas = PLATFORM_SKILL_ROOTS.map((root) => filesByRoot.get(root)?.get(relative)).filter((file): file is string => file !== undefined);
    ...
    if (canonicalDigest !== undefined && replicaDigests.every((digest) => digest === canonicalDigest)) {
      for (const replica of replicas) exclusions.add(replica);
      continue;
    }
    const retainedDigest = replicaDigests[0];
    if (retainedDigest === undefined) continue;
    for (let index = 1; index < replicas.length; index += 1) {
      if (replicaDigests[index] === retainedDigest) exclusions.add(replicas[index]!);
    }
  }
...
async function planTools(
  ...
  const omissions = await generatedBundleOmissions(
    input.repo,
    input.assetsRoot,
    input.mergeBaseSha,
    reviewableTypeScript,
  );
  ...
  if (ts.length) {
    ...
    if (input.includeRepositoryTools !== false && !repositoryTypecheckConfigured) plans.push(...await typescriptPlans(input, ts));
  }
  ...
  if (tsApplicable && !ts.length) {
    ...
    if (input.includeRepositoryTools !== false && !repositoryTypecheckConfigured) plans.push(...await typescriptPlans(input, relevantChangedFiles));
  }
  ...
  if (py.length) {
    ...
    if (input.includeRepositoryTools !== false) plans.push(...await pythonProjectPlans(input, py));
  }
  ...
  if (ts.length || py.length) {
    ...
    plans.push(...batches.map((batch, index) => ({
      ...
      outputContract: "semgrep-wasm" as const,
      files: batch,
      executable: process.execPath,
...
async function collectInventory(input: Input): Promise<ToolRunRecord> {
...
export async function collectTools(input: Input): Promise<ToolRunRecord[]> {
  ...
  for (const tool of ordered) {
    ...
    if (tool.internal === "oxlint-wasm" && record.status === "completed") {
      const raw = JSON.parse(await readFile(path.join(input.runDirectory, "deterministic", tool.name, "stdout.json"), "utf8")) as {
        files?: Array<{ path?: unknown; diagnostics?: unknown[] }>;
      };
      for (const file of raw.files ?? []) {
        ...
        const hasSyntaxFailure = file.diagnostics.some((diagnostic) => {
          ...
          const value = diagnostic as { severity?: unknown; code?: unknown };
          return value.severity === "error" && typeof value.code !== "string";
        });
        if (hasSyntaxFailure) syntaxFailures.add(file.path);
      }
    }
    if (tool.internal === "ruff-wasm" && record.status === "completed") {
      ...
      for (const file of raw.files ?? []) {
        ...
        const hasSyntaxFailure = file.diagnostics.some((diagnostic) => {
          if (diagnostic === null || typeof diagnostic !== "object") return false;
          return (diagnostic as { code?: unknown }).code === "invalid-syntax";
        });
        if (hasSyntaxFailure) syntaxFailures.add(file.path);
      }
    }
    ...
    records.push(record);
    if (tool.projectControlled) await input.afterRepositoryTool?.();
  }
  return records;
}

async function collectTool(
  input: Input,
  tool: Plan,
): Promise<ToolRunRecord> {
    const directory = path.join(input.runDirectory, "deterministic", tool.name);
    await ensureDirectory(directory);
    if (tool.internal === "generated-bundle-omissions") return collectGeneratedBundleOmissions(input, tool, directory);
    if (tool.internal === "oxlint-wasm") return collectOxlintWasm(input, tool, directory);
    if (tool.internal === "ruff-wasm") return collectRuffWasm(input, tool, directory);
    if (!tool.executable) {
    ...
    try {
      ...
      if (result.timedOut) record.reason = "Command exceeded its configured timeout";
      else if (result.outputLimitExceeded) record.reason = "Command exceeded the output capture limit";
      else if (result.spawnError) record.reason = result.spawnError;
      else if (result.versionFailure) record.reason = result.versionFailure;
      else if (outputFailure) record.reason = outputFailure;
      else if (operationalFailure) record.reason = `Unexpected tool exit code ${result.exitCode}`;
      return record;
    } finally {
      if (tool.cleanupDirectory) await rm(tool.cleanupDirectory, { recursive: true, force: true });
    }
}

async function collectGeneratedBundleOmissions(input: Input, tool: Plan, directory: string): Promise<ToolRunRecord> {
...
async function collectRuffWasm(input: Input, tool: Plan, directory: string): Promise<ToolRunRecord> {
  ...
  try {
    ...
    if (cancelled) throw new FriendlyAdversaryError(reason, exitCode);
    return {
      name: tool.name,
...
async function collectOxlintWasm(input: Input, tool: Plan, directory: string): Promise<ToolRunRecord> {
  const startedAt = Date.now();
  const files = tool.files ?? [];
