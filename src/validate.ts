...
async function validateOxlintArtifact(errors: string[], root: string, relative: string): Promise<void> {
  ...
  if (digest !== OXLINT_SHA256) errors.push(`${relative}: SHA-256 ${digest} does not match ${OXLINT_SHA256}`);
}

async function validateRuffArtifact(errors: string[], root: string): Promise<void> {
  const runtimeRoot = path.join(root, "engines", "ruff-wasm", "runtime");
  for (const [file, expected] of [["ruff_wasm_bg.wasm", RUFF_WASM_SHA256], ["ruff_wasm.js", RUFF_GLUE_SHA256]] as const) {
    ...
    const digest = createHash("sha256").update(artifact).digest("hex");
    if (digest !== expected) errors.push(`${relative}: SHA-256 ${digest} does not match ${expected}`);
  }
}

async function validateRipgrepArtifact(errors: string[], root: string): Promise<void> {
  ...
  const digest = createHash("sha256").update(artifact).digest("hex");
  if (digest !== RIPGREP_WASM_SHA256) errors.push(`${relative}: SHA-256 ${digest} does not match ${RIPGREP_WASM_SHA256}`);
  try {
...
async function validateRipgrepDistribution(errors: string[], root: string): Promise<void> {
  ...
  for (const line of (await readFile(sumsPath, "utf8")).trim().split(/\r?\n/u)) {
    ...
    if (expected.has(match[2])) errors.push(`${relativeRoot}/SHA256SUMS: duplicate entry ${match[2]}`);
  ...
  for (const file of required) if (!actual.includes(file)) errors.push(`${relativeRoot}/${file}: required distribution evidence is missing`);
  if (actual.length < 60) errors.push(`${relativeRoot}: incomplete distribution inventory (${actual.length} files)`);
  for (const file of actual) {
    const digest = createHash("sha256").update(await readFile(path.join(engineRoot, ...file.split("/")))).digest("hex");
    if (expected.get(file) !== digest) errors.push(`${relativeRoot}/${file}: missing or mismatched checksum`);
  }
  for (const file of expected.keys()) if (!actual.includes(file)) errors.push(`${relativeRoot}/${file}: listed but missing`);
  try {
    const lock = JSON.parse(await readFile(path.join(engineRoot, "upstream-lock.json"), "utf8")) as {
      upstream?: { tag?: string; commit?: string; sourceArchive?: { sha256?: string } };
      build?: { rustc?: string; rustcCommit?: string; target?: string; sourcePatches?: number; pcre2Enabled?: boolean };
      runtime?: { sha256?: string; imports?: string[] };
    ...
    for (const [name, digest] of Object.entries(noticeHashes)) {
      if (lock.notices?.[name] !== digest) errors.push(`${relativeRoot}/upstream-lock.json: unexpected ${name}`);
    }
  } catch (error) {
...
function metadataList(value: string | undefined): string[] | undefined {
  if (!value || !/^\[[^\]]+\]$/.test(value)) return undefined;
  const values = value.slice(1, -1).split(",").map((entry) => entry.trim());
  if (!values.length || values.some((entry) => !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry))) return undefined;
  return values;
}
...
async function walk(root: string, current = root): Promise<string[]> {
  ...
  for (const entry of await readdir(current, { withFileTypes: true })) {
    ...
    if ([".git", ".friendly-adversary", "node_modules", "dist", "build/oxlint-wasm/target"].some((directory) => relative === directory || relative.startsWith(`${directory}/`))) continue;
    if (entry.isDirectory()) files.push(...await walk(root, absolute)); else files.push(path.relative(root, absolute));
  }
  return files;
}

async function validateThirdPartyNotices(errors: string[], root: string): Promise<void> {
  ...
  for (const line of (await readFile(sumsPath, "utf8")).trim().split(/\r?\n/u)) {
    ...
    if (expected.has(match[2])) errors.push(`third-party/oxlint-wasm/SHA256SUMS: duplicate entry ${match[2]}`);
    expected.set(match[2], match[1]);
  }
  const actual = (await walk(noticeRoot)).map((file) => file.split(path.sep).join("/")).filter((file) => file !== "SHA256SUMS").sort();
  if (!actual.includes("NOTICE.md") || actual.length < 165) errors.push(`third-party/oxlint-wasm: incomplete notice inventory (${actual.length} files)`);
  const notice = await readFile(path.join(noticeRoot, "NOTICE.md"), "utf8").catch(() => "");
  if (!notice.includes("| Upstream license expression | Selected compatible license |")) errors.push("third-party/oxlint-wasm/NOTICE.md: missing compatible-license selections");
  const selections = new Map<string, string>();
  for (const line of notice.split(/\r?\n/u)) {
    const match = /^\| `([^`]+)` \| `([^`]+)` \|$/u.exec(line);
    if (match?.[1] && match[2]) {
      if (selections.has(match[1])) errors.push(`third-party/oxlint-wasm/NOTICE.md: duplicate license selection for ${match[1]}`);
  ...
  if (noticeRows.length !== 164) errors.push(`third-party/oxlint-wasm/NOTICE.md: expected 164 dependency rows, found ${noticeRows.length}`);
  ...
  for (const file of actual) {
    ...
    if (expected.get(file) !== digest) errors.push(`third-party/oxlint-wasm/${file}: missing or mismatched checksum`);
  }
  for (const file of expected.keys()) if (!actual.includes(file)) errors.push(`third-party/oxlint-wasm/${file}: listed but missing`);
}

async function validatePortableFiles(errors: string[], root: string): Promise<void> {
  ...
  for (const file of await walk(root)) {
    ...
    if (path.basename(file) === "friendly-adversary-mcp.cjs") continue;
    ...
    if (content.includes(forbiddenPunctuation)) errors.push(`${file}: contains forbidden punctuation`);
  }
}

export function validateMcpManifestContract(platform: "claude-code" | "codex", value: unknown): string[] {
...
function validateLens(errors: string[], id: string, content: string): void {
  const values = metadata(content);
  if (values.id !== id) errors.push(`${id}: frontmatter id differs from path`);
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id)) errors.push(`${id}: unsafe lens identifier`);
  if (values.version !== "1") errors.push(`${id}: unsupported lens contract version`);
  for (const key of ["title", "version", "languages", "applies_to", "evidence", "evaluation_tags"]) if (!values[key]) errors.push(`${id}: missing ${key}`);
  ...
  for (const key of ["applies_to", "evidence", "evaluation_tags"] as const) {
    const entries = metadataList(values[key]);
    if (!entries) errors.push(`${id}: ${key} must be a non-empty identifier list`);
    else if (new Set(entries).size !== entries.length) errors.push(`${id}: ${key} contains duplicates`);
  }
  if (content.split(/\r?\n/).length > 250) errors.push(`${id}: exceeds the 250-line limit`);
  const frontmatter = /^---\r?\n[\s\S]*?\r?\n---\r?\n/u.exec(content);
  const firstBodyLine = !frontmatter
    ? ""
    : content.slice(frontmatter[0].length).split(/\r?\n/u).find((line) => line.trim())?.trim();
  if (title && firstBodyLine !== `# ${title}`) errors.push(`${id}: first body heading must be '# ${title}'`);
  let previous = -1;
  for (const section of SECTIONS) {
    const at = content.indexOf(section);
    if (at < 0) errors.push(`${id}: missing ${section}`);
    else if (at < previous) errors.push(`${id}: ${section} is out of order`);
    previous = Math.max(previous, at);
  }
  if (!sectionBody(content, "## Valid finding example")) errors.push(`${id}: valid finding example is empty`);
  if (!sectionBody(content, "## Invalid finding example")) errors.push(`${id}: invalid finding example is empty`);
}
...
function validateNonEditingSkillContract(errors: string[], label: string, content: string): void {
  for (const requirement of NON_EDITING_SKILL_REQUIREMENTS) {
    if (!content.includes(requirement)) errors.push(`${label}: missing non-editing requirement: ${requirement}`);
  }
}

function validateOrchestrationSkillContract(errors: string[], label: string, content: string): void {
  for (const requirement of ORCHESTRATION_SKILL_REQUIREMENTS) {
    if (!content.includes(requirement)) errors.push(`${label}: missing orchestration requirement: ${requirement}`);
  }
}
...
function reviewedSkillDigest(content: string): string {
  return createHash("sha256").update(content.replaceAll("\r\n", "\n")).digest("hex");
}

function validateCodexSkillContract(errors: string[], label: string, content: string): void {
  if (reviewedSkillDigest(content) !== CODEX_SKILL_SHA256) {
    errors.push(`${label}: differs from the reviewed Codex skill contract`);
  }
}

function validateClaudeSkillContract(errors: string[], label: string, content: string): void {
  if (reviewedSkillDigest(content) !== CLAUDE_SKILL_SHA256) {
    errors.push(`${label}: differs from the reviewed Claude Code skill contract`);
  }
}

function validateCodexToolingContract(errors: string[], label: string, content: string): void {
  const normalizedContent = content.replaceAll("\r\n", "\n");
  const digest = createHash("sha256").update(normalizedContent).digest("hex");
  if (digest !== CODEX_TOOLING_SHA256) errors.push(`${label}: differs from the reviewed Codex tooling contract`);
}

function validateCodexAgentMarker(errors: string[], label: string, content: string): void {
  const digest = createHash("sha256").update(content.replaceAll("\r\n", "\n")).digest("hex");
  if (digest !== CODEX_AGENT_MARKER_SHA256) errors.push(`${label}: differs from the reviewed Codex platform marker`);
}

function validateAdjudicationContract(errors: string[], label: string, content: string): void {
  for (const requirement of ADJUDICATION_REQUIREMENTS) {
    if (!content.includes(requirement)) errors.push(`${label}: missing adjudication requirement: ${requirement}`);
  }
}

function validateFindingContract(errors: string[], label: string, content: string): void {
...
export async function validateRepository(root: string): Promise<string[]> {
  ...
  if (!await pathExists(lensRoot) && await pathExists(bundledLensRoot)) {
    ...
    for (const id of IDS) if (!actual.includes(id)) errors.push(`Missing bundled lens: ${id}`);
    for (const file of lensFiles) validateLens(errors, file.slice(0, -3), await readFile(path.join(bundledLensRoot, file), "utf8"));
    for (const required of ["LICENSE", "LICENSING.md", "package.json", "SKILL.md", "scripts/runtime/cli.js", "scripts/runtime/wasm/oxlint/engine.wasm", "scripts/runtime/wasm/oxlint/LICENSE-OXC-MIT.txt", "third-party/oxlint-wasm/NOTICE.md", "engines/ruff-wasm/runtime/ruff_wasm.js", "engines/ruff-wasm/runtime/ruff_wasm_bg.wasm", "engines/ruff-wasm/LICENSE", "engines/ruff-wasm/upstream-lock.json", "engines/ripgrep-wasm/runtime/rg.wasm", "engines/ripgrep-wasm/upstream-lock.json", "engines/ripgrep-wasm/SHA256SUMS", "engines/ripgrep-wasm/THIRD_PARTY_LICENSES.md", "engines/semgrep-wasm/runtime-manifest.json", "engines/semgrep-wasm/runtime/engine/semgrep-engine.wasm", "engines/semgrep-wasm/source/generated-parser-license-evidence.json", "engines/semgrep-wasm/source/licenses/GENERATED-PARSERS-GPL-3.0.txt", "engines/semgrep-wasm/source/licenses/SEMGREP-LGPL-2.1.txt", "rules/semgrep/javascript.yml", "rules/semgrep/python.yml", "references/adjudication.md", "references/finding-contract.md"]) {
      if (!await pathExists(path.join(root, required))) errors.push(`Missing bundled file: ${required}`);
    }
    try {
      const runtimePackage = JSON.parse(await readFile(path.join(root, "package.json"), "utf8")) as {
        private?: unknown;
        type?: unknown;
      };
      if (runtimePackage.private !== true) errors.push("package.json must mark the installed skill runtime private");
      if (runtimePackage.type !== "module") errors.push("package.json must mark the installed skill runtime as an ES module");
    } catch (error) {
    ...
    if (hasCodexMarker) {
      validateCodexSkillContract(errors, "SKILL.md", installedSkill);
      validateCodexForkIsolation(errors, "SKILL.md", installedSkill);
      validateCodexAgentMarker(errors, "agents/openai.yaml", await readFile(codexMarker, "utf8"));
    ...
    if (errors.length) throw new FriendlyAdversaryError(`Plugin bundle validation failed:\n${errors.map((error) => `- ${error}`).join("\n")}`, 2);
  ...
  for (const id of IDS) if (!actual.includes(id)) errors.push(`Missing lens: ${id}`);
  if (actual.length < IDS.length) errors.push(`Expected at least ${IDS.length} lenses, found ${actual.length}`);
  for (const id of actual) {
    const file = path.join(lensRoot, id, "LENS.md");
    if (!await pathExists(file)) { errors.push(`Missing LENS.md for ${id}`); continue; }
  ...
  for (const id of agents) if (!workflowAgents.has(id)) errors.push(`Claude Code agent has no workflow role: ${id}`);
  for (const id of workflowAgents) if (!agents.includes(id)) errors.push(`Missing Claude Code workflow agent: ${id}`);
  for (const platform of ["claude-code", "codex"]) {
    ...
    if (platform === "codex") {
      validateCodexSkillContract(errors, skillRelative, skill);
      validateCodexForkIsolation(errors, skillRelative, skill);
      const agentRelative = path.join(skillRoot, "agents", "openai.yaml");
      if (!await pathExists(path.join(root, agentRelative))) errors.push(`${agentRelative}: missing Codex platform marker`);
      else validateCodexAgentMarker(errors, agentRelative, await readFile(path.join(root, agentRelative), "utf8"));
    } else validateClaudeSkillContract(errors, skillRelative, skill);
    ...
    for (const skillName of ["audit-codebase", "design-new-codebase"]) {
      ...
      if (values.name !== skillName) errors.push(`${supplementalRelative}: frontmatter name differs from path`);
      ...
      if (!supplemental.includes("record_artifact")) errors.push(`${supplementalRelative}: missing the single artifact tool contract`);
      ...
      if (skillName === "audit-codebase") {
        ...
        if (!supplemental.includes(required)) errors.push(`${supplementalRelative}: missing parallel-wait requirement: ${required}`);
    ...
    if (await pathExists(retiredRoot)) errors.push(`${path.relative(root, retiredRoot)}: retired skill must not remain installed`);
  }
  ...
  for (const required of [...requiredFiles, ...requiredJson]) if (!await pathExists(path.join(root, required))) errors.push(`Missing required file: ${required}`);
  ...
  await validateThirdPartyNotices(errors, root);
  await validatePortableFiles(errors, root);
  if (errors.length) throw new FriendlyAdversaryError(`Repository validation failed:\n${errors.map((error) => `- ${error}`).join("\n")}`, 2);
