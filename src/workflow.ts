...
import {
  authorityRoot,
  cleanupExpiredAuthorities,
  DEFAULT_CAPABILITY_LIFETIME_MS,
  ensureAuthorityRoot,
  publishAuthorityControlFile,
  readAuthorityControlFile,
  type ReviewHost,
} from "./authority.js";
...
import { acquireRunLock, releaseRunLock, type RunLock, type RunLockOperation } from "./run-lock.js";
import { detectRecognizableSecret } from "./secret-patterns.js";
import type { ToolRunRecord } from "./types.js";

export type WorkflowKind = "audit-codebase" | "design-new-codebase";
export type WorkflowStatus = "prepared" | "planned" | "incomplete" | "ready" | "sealed" | "sealed-incomplete" | "aborted";

export interface WorkflowSnapshot {
  mode: "git-files" | "directory-identity";
  root: string;
  headSha: string | null;
  digest: string;
  fileCount: number;
  files: Array<{ path: string; kind: "file" | "symlink" | "gitlink" | "deleted"; mode: number; bytes: number; sha256: string; target?: string; gitObject?: string; gitStatus?: string }>;
  index: Array<{ path: string; mode: string; stage: number; object: string }>;
}

export interface WorkflowLane {
  id: string;
  kind: "subsystem" | "decision" | "research" | "challenge";
  title: string;
  scope: string;
  dimensions: string[];
}
...
export interface WorkflowReceipt {
  ...
  publicationSlots: Record<string, PublicationSlot>;
  receiptGeneration: number;
  pendingDecisionRevisions: Record<string, string>;
  sealedAt?: string;
  artifactCount?: number;
}

interface WorkflowAuthorityScope {
  digest: string;
  exactPaths: string[];
  prefixPaths: string[];
  maxArtifacts: number;
...
interface WorkflowAuthorityRecord {
  schemaVersion: "1";
  productVersion: string;
  authorityId: string;
  runId: string;
  ...
  receiptSlots: Record<string, PublicationSlot>;
  createdAt: string;
  expiresAt: string;
  scopes: Record<string, WorkflowAuthorityScope>;
}

export interface WorkflowAuthorityPacket {
...
function throwIfWorkflowCancelled(signal?: AbortSignal): void {
  if (signal?.aborted) throw new FriendlyAdversaryError("FA_CANCELLED: tool call was cancelled before publication", 3);
}
...
function authorityFile(authorityId: string): string {
  if (!AUTHORITY_ID.test(authorityId)) throw new FriendlyAdversaryError("FA_AUTHORITY_INVALID: invalid workflow authority ID", 2);
  return path.join(authorityRoot(), `${authorityId}.workflow.json`);
}

function workflowAuthorityPrefix(runDirectory: string): string {
  return sha256(path.resolve(runDirectory)).slice(0, 16);
}

function sealTransactionFile(runDirectory: string): string {
  return path.join(authorityRoot(), sha256(`${SEAL_TRANSACTION_DOMAIN}\0${path.resolve(runDirectory)}`).slice(0, 32) + ".json");
}

function sealReceiptDigest(receipt: WorkflowReceipt): string {
  return sha256(`${SEAL_TRANSACTION_DOMAIN}\0${canonical(receipt)}`);
}

async function writeSealTransaction(receipt: WorkflowReceipt, manifestDigest: string): Promise<void> {
...
async function sealTransactionMatches(receipt: WorkflowReceipt, manifest: string): Promise<boolean> {
  try {
    const value = JSON.parse(await readAuthorityControlFile(
      sealTransactionFile(receipt.outputDirectory),
      "FA_SEAL_TRANSACTION_NOT_FOUND",
      "FA_SEAL_TRANSACTION_UNSAFE",
    )) as unknown;
    if (!isPlainRecord(value) || Object.keys(value).sort().join(",") !== "manifestDigest,receiptDigest,runDirectory,runId,schemaVersion") return false;
...
async function readWorkflowAuthority(authorityId: string): Promise<WorkflowAuthorityRecord> {
  let parsed: unknown;
  try {
    parsed = JSON.parse(await readAuthorityControlFile(authorityFile(authorityId))) as unknown;
  } catch (error) {
    if (error instanceof FriendlyAdversaryError) throw error;
...
function isPublicationSlot(value: unknown): value is PublicationSlot {
  if (!isPlainRecord(value) || Object.keys(value).sort().join(",") !== "device,inode,parentDevice,parentInode") return false;
...
function isValidAuthorityArtifactPath(value: unknown): value is string {
...
function validateWorkflowAuthorityRecord(value: unknown, authorityId: string): WorkflowAuthorityRecord {
  if (!isPlainRecord(value)) throw new FriendlyAdversaryError("FA_AUTHORITY_INCOMPATIBLE: workflow authority is malformed", 2);
  ...
  for (const [scope, candidate] of Object.entries(value.scopes)) {
    if (!scope || !isPlainRecord(candidate) || Object.keys(candidate).sort().join(",") !== "digest,exactPaths,maxArtifacts,prefixPaths"
      ...
      || !Number.isSafeInteger(candidate.maxArtifacts) || (candidate.maxArtifacts as number) < 1) {
      throw new FriendlyAdversaryError("FA_AUTHORITY_INCOMPATIBLE: workflow authority scope is malformed", 2);
    }
  }
  return value as unknown as WorkflowAuthorityRecord;
}

function capabilityDigest(token: string, record: Omit<WorkflowAuthorityRecord, "scopes">, scope: string): string {
...
async function reservePublicationSlot(runDirectory: string, relativePath: string): Promise<PublicationSlot> {
  if (relativePath.includes("/") || relativePath.includes("\\")) throw new FriendlyAdversaryError("FA_PUBLICATION_SLOT_INVALID: workflow slots must be flat", 2);
  const parentBefore = await lstat(runDirectory, { bigint: true });
  if (!parentBefore.isDirectory() || parentBefore.isSymbolicLink()) throw new FriendlyAdversaryError("FA_PUBLICATION_PARENT_CHANGED: run root is unsafe", 3);
  const target = path.join(runDirectory, relativePath);
  const handle = await open(target, constants.O_CREAT | constants.O_EXCL | constants.O_RDWR | (constants.O_NOFOLLOW ?? 0), 0o600).catch((error: NodeJS.ErrnoException) => {
...
async function reservePublicationSlots(receipt: WorkflowReceipt, relativePaths: string[], persist = false): Promise<void> {
  ...
  if (persist) await writeReceipt(receipt);
}

function markdownEnvelope(markdown: string): string {
  const body = `${markdown.trimEnd()}\n`;
  return `${body}${ARTIFACT_COMPLETE} sha256=${sha256(body)} -->\n`;
}

function stripMarkdownEnvelope(markdown: string): string {
  ...
  if (!/^[a-f0-9]{64}$/u.test(digest)) return markdown.trimEnd();
  return markdown.slice(0, markerStart + 1).trimEnd();
}

function manifestEnvelope(manifest: string): string {
...
function manifestBody(manifest: string): string {
  ...
  if (!safeEqual(sha256(body), digest)) throw new FriendlyAdversaryError("FA_MANIFEST_INVALID: artifact manifest completion hash is invalid", 2);
  return body;
}

function committedSlotContent(relativePath: string, content: Buffer): boolean {
  if (!content.byteLength) return false;
  ...
  if (relativePath.endsWith(".html")) return content.toString("utf8").trimEnd().endsWith("</html>");
  if (relativePath === MANIFEST) {
    try { manifestBody(content.toString("utf8")); return true; } catch { return false; }
  }
  return false;
}

async function readHandle(handle: Awaited<ReturnType<typeof open>>, size: number): Promise<Buffer> {
...
async function publishReserved(receipt: WorkflowReceipt, relativePath: string, content: string | Uint8Array): Promise<"created" | "confirmed_existing" | "recovered_transaction"> {
  const slot = receipt.publicationSlots[relativePath];
  if (!slot) throw new FriendlyAdversaryError(`FA_PUBLICATION_SLOT_MISSING: ${relativePath}`, 2);
  ...
  try {
    ...
    if (previous.equals(desired)) return "confirmed_existing";
    if (committedSlotContent(relativePath, previous)) throw new FriendlyAdversaryError("FA_ARTIFACT_CONFLICT: artifact already contains a different committed publication", 2);
...
async function preflightReservedPublication(receipt: WorkflowReceipt, relativePath: string, content: string | Uint8Array): Promise<void> {
  const slot = receipt.publicationSlots[relativePath];
  if (!slot) throw new FriendlyAdversaryError(`FA_PUBLICATION_SLOT_MISSING: ${relativePath}`, 2);
  const target = path.join(receipt.outputDirectory, relativePath);
  ...
  try {
    ...
    if (previous.equals(desired) || !committedSlotContent(relativePath, previous)) return;
...
async function writeReservedMutable(receipt: WorkflowReceipt, relativePath: string, content: string | Uint8Array): Promise<void> {
  const slot = receipt.publicationSlots[relativePath];
  if (!slot) throw new FriendlyAdversaryError(`FA_PUBLICATION_SLOT_MISSING: ${relativePath}`, 2);
  const target = path.join(receipt.outputDirectory, relativePath);
...
async function readReceiptCandidate(runDirectory: string, name: string): Promise<WorkflowReceipt | undefined> {
  const candidatePath = path.join(runDirectory, name);
  try {
    const receipt = JSON.parse(await readFile(candidatePath, "utf8")) as WorkflowReceipt;
    ...
    if (receipt.schemaVersion !== "1" || receipt.productVersion !== PRODUCT_VERSION || receipt.outputDirectory !== runDirectory
      ...
      || !slot || ![slot.device, slot.inode, slot.parentDevice, slot.parentInode].every((value) => /^[0-9]+$/u.test(value))) return undefined;
    ...
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1n
      ...
      || parent.dev !== BigInt(slot.parentDevice) || parent.ino !== BigInt(slot.parentInode)) return undefined;
    return receipt;
  } catch {
    return undefined;
  }
...
async function writeReceipt(receipt: WorkflowReceipt): Promise<void> {
  const candidates = (await Promise.all([
    readReceiptCandidate(receipt.outputDirectory, RECEIPT),
    readReceiptCandidate(receipt.outputDirectory, RECEIPT_ALTERNATE),
  ])).filter((candidate): candidate is WorkflowReceipt => candidate !== undefined)
...
function validateRelativeArtifact(relativePath: string): string {
  ...
  if (!relativePath.endsWith(".md")) throw new FriendlyAdversaryError("FA_ARTIFACT_TYPE_INVALID: agent-authored artifacts must be Markdown", 2);
  return relativePath;
}

function validateMarkdown(markdown: string): void {
  const bytes = Buffer.byteLength(markdown, "utf8");
  if (!markdown.trim() || bytes > MAX_ARTIFACT_BYTES) throw new FriendlyAdversaryError("FA_ARTIFACT_INVALID: Markdown is empty or exceeds the size limit", 2);
  if (detectRecognizableSecret(markdown)) throw new FriendlyAdversaryError("FA_ARTIFACT_SECRET: Markdown contains credential-like material", 2);
}

function workflowGitBuffer(repositoryRoot: string, args: string[]): Buffer {
  try {
    ...
    throw new FriendlyAdversaryError(
      `Git command failed while pinning the workflow snapshot: git ${args.join(" ")}\n${stderr.trim()}`,
      failure.status ?? 2,
    );
  }
}
...
function gitlinkContent(repositoryRoot: string, relative: string): { content: Buffer; object: string; status: string } {
  const staged = gitText(repositoryRoot, ["ls-files", "--stage", "--", relative]).trim();
  const match = /^160000 ([a-f0-9]{40,64}) 0\t/u.exec(staged);
  if (!match?.[1]) throw new FriendlyAdversaryError(`FA_SNAPSHOT_UNSUPPORTED: tracked directory is not a valid Git submodule: ${relative}`, 2);
...
async function assertSourcePathConfined(repositoryRoot: string, relative: string): Promise<void> {
...
async function assertCanonicalSourceInside(repositoryRoot: string, absolute: string, relative: string): Promise<void> {
  const canonical = await realpath(absolute);
  const contained = path.relative(repositoryRoot, canonical);
  if (path.isAbsolute(contained) || contained.startsWith("..")) throw new FriendlyAdversaryError(`FA_SNAPSHOT_UNSAFE: tracked path resolves outside the repository: ${relative}`, 2);
}

export function reserveWorkflowSnapshotBytes(totalBytes: number, fileBytes: number, relative: string): number {
...
function gitIndex(repositoryRoot: string): WorkflowSnapshot["index"] {
...
async function auditSnapshot(candidate: string, materializeRoot?: string): Promise<WorkflowSnapshot> {
  const root = await resolveRepositoryRoot(candidate);
  const files = monitoredGitPaths(root);
  const records: WorkflowSnapshot["files"] = [];
  const digest = createHash("sha256");
  const headSha = gitHead(root);
  let totalBytes = 0;
  digest.update(`head\0${headSha}\0`);
  const index = gitIndex(root);
  for (const entry of index) digest.update(`index\0${entry.mode}\0${entry.object}\0${entry.stage}\0${entry.path}\0`);
  if (materializeRoot) await ensureDirectory(materializeRoot);
  for (const relative of files) {
    ...
    if (!metadata) {
      const record = { path: relative, kind: "deleted" as const, mode: 0, bytes: 0, sha256: sha256("") };
      records.push(record);
    ...
    if (metadata.isSymbolicLink()) {
      ...
      const record = { path: relative, kind: "symlink" as const, mode, bytes: content.byteLength, sha256: sha256(content), target };
      records.push(record);
      ...
      const record = { path: relative, kind: "file" as const, mode, bytes: content.byteLength, sha256: sha256(content) };
      records.push(record);
      digest.update(`file\0${relative}\0${record.mode}\0${record.sha256}\0${record.bytes}\0`);
      if (materializeRoot) await writeFileAtomic(path.join(materializeRoot, ...relative.split("/")), content);
    } else if (metadata.isDirectory()) {
      await assertCanonicalSourceInside(root, absolute, relative);
      const gitlink = gitlinkContent(root, relative);
      const content = gitlink.content;
      const record = { path: relative, kind: "gitlink" as const, mode, bytes: content.byteLength, sha256: sha256(content), gitObject: gitlink.object, gitStatus: gitlink.status };
      records.push(record);
...
async function directorySnapshot(candidate: string): Promise<WorkflowSnapshot> {
  ...
  if (!metadata.isDirectory()) throw new FriendlyAdversaryError("FA_DESIGN_ROOT_INVALID: the design root must be a directory", 2);
...
async function currentSnapshot(receipt: WorkflowReceipt): Promise<WorkflowSnapshot> {
  return receipt.kind === "audit-codebase" ? auditSnapshot(receipt.root) : directorySnapshot(receipt.root);
}

async function verifyMaterializedSnapshot(receipt: WorkflowReceipt): Promise<void> {
  ...
  for (const file of expected) {
    ...
    if (!metadata?.isFile() || metadata.isSymbolicLink()) throw new FriendlyAdversaryError(`FA_SNAPSHOT_MATERIALIZATION_STALE: ${file.path}`, 3);
    const content = await readFile(candidate);
    if (content.byteLength !== file.bytes || sha256(content) !== file.sha256) throw new FriendlyAdversaryError(`FA_SNAPSHOT_MATERIALIZATION_STALE: ${file.path}`, 3);
  }
}

async function readReceipt(runDirectory: string): Promise<WorkflowReceipt> {
  ...
  const candidates = (await Promise.all([
    ...
    .sort((left, right) => right.receiptGeneration - left.receiptGeneration);
  if (!candidates[0]) throw new FriendlyAdversaryError("FA_RUN_NOT_FOUND: workflow receipt generations are missing or incomplete", 2);
  return candidates[0];
}

const WORKFLOW_LOCK_WAIT_MS = 60_000;

async function acquireWorkflowLock(receipt: WorkflowReceipt, operation: RunLockOperation): Promise<RunLock> {
  ...
  for (;;) {
    try {
      ...
      if (!(error instanceof FriendlyAdversaryError) || !error.message.startsWith("FA_RUN_BUSY:") || Date.now() >= deadline) throw error;
      await new Promise<void>((resolve) => setTimeout(resolve, 25));
    }
  }
}

async function withWorkflowMutation<T>(
  runDirectory: string,
  operation: RunLockOperation,
  mutate: (receipt: WorkflowReceipt) => Promise<T>,
): Promise<T> {
...
async function createAuthority(receipt: WorkflowReceipt, scopes: Record<string, { exactPaths?: string[]; prefixPaths?: string[]; maxArtifacts?: number }>): Promise<WorkflowAuthorityPacket> {
  ...
  const base: Omit<WorkflowAuthorityRecord, "scopes"> = {
    schemaVersion: "1",
  ...
  const stored: Record<string, WorkflowAuthorityScope> = {};
...
async function validateAuthority(authorityId: string, capability: string, relativePath: string): Promise<{ receipt: WorkflowReceipt; scope: string; record: WorkflowAuthorityRecord }> {
  if (!CAPABILITY.test(capability)) throw new FriendlyAdversaryError("FA_CAPABILITY_INVALID: invalid workflow capability", 2);
  ...
  for (const [scope, limits] of Object.entries(scopes)) {
    if (!safeEqual(capabilityDigest(capability, base, scope), limits.digest)) continue;
    const allowed = limits.exactPaths.includes(relativePath) || limits.prefixPaths.some((prefix) => relativePath.startsWith(prefix));
    if (!allowed) throw new FriendlyAdversaryError("FA_CAPABILITY_DENIED: capability does not authorize this artifact path", 2);
    const existingPaths = new Set<string>();
    for (const exact of limits.exactPaths) {
      if (scope === "plan" ? await pathExists(path.join(record.runDirectory, exact)) : await publicationExists(record.runDirectory, scope, exact)) existingPaths.add(exact);
    }
    for (const prefix of limits.prefixPaths) {
      for (const entry of await readdir(record.runDirectory, { withFileTypes: true })) {
        if (entry.isFile() && entry.name.startsWith(prefix) && entry.name.endsWith(".md") && await publicationExists(record.runDirectory, scope, entry.name)) existingPaths.add(entry.name);
      }
    }
  ...
  if (!matched) throw new FriendlyAdversaryError("FA_CAPABILITY_DENIED: capability does not authorize this workflow", 2);
  ...
  if (receipt.kind === "audit-codebase") await verifyMaterializedSnapshot(receipt);
  return { receipt, scope: matched, record };
}

function runPrefix(kind: WorkflowKind): string {
  return `${new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-")}-${kind}-${randomUUID().slice(0, 8)}`;
}

async function startWorkflow(kind: WorkflowKind, candidate: string, host: ReviewHost, timeoutMs: number): Promise<{ receipt: WorkflowReceipt; authority: WorkflowAuthorityPacket }> {
...
export async function startAudit(input: { repo: string; host: ReviewHost; timeoutMs?: number }): Promise<{ receipt: WorkflowReceipt; authority: WorkflowAuthorityPacket }> {
  return startWorkflow("audit-codebase", input.repo, input.host, input.timeoutMs ?? DEFAULT_TIMEOUT_MS);
}

export async function startDesign(input: { root: string; host: ReviewHost }): Promise<{ receipt: WorkflowReceipt; authority: WorkflowAuthorityPacket }> {
  return startWorkflow("design-new-codebase", input.root, input.host, DEFAULT_TIMEOUT_MS);
}

function validateManifest(kind: WorkflowKind, manifest: WorkflowManifest): WorkflowLane[] {
  if (!Array.isArray(manifest.lanes) || !manifest.lanes.length) throw new FriendlyAdversaryError("FA_PLAN_INVALID: workflow plan requires at least one lane", 2);
  ...
  if (kind === "design-new-codebase") {
    ...
    if (missing.length) throw new FriendlyAdversaryError(`FA_PLAN_INVALID: design plan is missing required challenges: ${missing.join(", ")}`, 2);
  }
  return lanes;
}

function lanePath(lane: WorkflowLane): string {
  if (lane.kind === "subsystem") return `subsystem-${lane.id}.md`;
  if (lane.kind === "research") return `research-${lane.id}.md`;
  if (lane.kind === "challenge") return `challenge-${lane.id}.md`;
  return `decision-${lane.id}-`;
}

function isPrimaryLane(kind: WorkflowKind, lane: WorkflowLane): boolean {
  return kind === "audit-codebase" ? lane.kind === "subsystem" : lane.kind === "decision" || lane.kind === "research";
}

function allPlannedArtifactPaths(kind: WorkflowKind, lanes: WorkflowLane[]): string[] {
  const agentPaths: string[] = [];
  for (const lane of lanes) {
    if (lane.kind === "decision") agentPaths.push(`${lanePath(lane)}0001.md`);
    else agentPaths.push(lanePath(lane));
  }
...
function publicationPath(runDirectory: string, relativePath: string): string {
  return path.join(runDirectory, `${PUBLICATION_PREFIX}${sha256(relativePath)}.json`);
}

async function recordPublication(receipt: WorkflowReceipt, scope: string, relativePath: string, content: string): Promise<void> {
  const record = {
    schemaVersion: "1",
    productVersion: PRODUCT_VERSION,
    runId: receipt.runId,
...
async function publicationExists(runDirectory: string, scope: string, relativePath: string): Promise<boolean> {
  try {
    const artifact = await readFile(path.join(runDirectory, relativePath));
    const record = JSON.parse(await readFile(publicationPath(runDirectory, relativePath), "utf8")) as {
      scope?: string;
      relativePath?: string;
      bytes?: number;
      sha256?: string;
...
export async function establishWorkflowPlan(input: { workflow?: WorkflowKind; authorityId: string; capability: string; overviewMarkdown: string; manifest: WorkflowManifest; signal?: AbortSignal }): Promise<WorkflowAuthorityPacket> {
  ...
  if (input.workflow && receipt.kind !== input.workflow) throw new FriendlyAdversaryError("FA_WORKFLOW_MISMATCH: authority belongs to another workflow", 2);
  if (receipt.status !== "prepared") throw new FriendlyAdversaryError("FA_PLAN_STATE_INVALID: workflow plan can only be established once", 2);
  ...
  receipt.updatedAt = new Date().toISOString();
  const scopes: Record<string, { exactPaths?: string[]; prefixPaths?: string[]; maxArtifacts?: number }> = {};
...
export async function recordWorkflowArtifact(input: { workflow?: WorkflowKind; authorityId: string; capability: string; relativePath: string; markdown: string; signal?: AbortSignal }): Promise<{ run_id: string; relative_path: string; publication: "created" | "confirmed_existing" | "recovered_transaction"; bytes: number; sha256: string }> {
  ...
  if (input.workflow && receipt.kind !== input.workflow) throw new FriendlyAdversaryError("FA_WORKFLOW_MISMATCH: authority belongs to another workflow", 2);
  if (!["planned", "incomplete"].includes(receipt.status)) throw new FriendlyAdversaryError("FA_RUN_STATE_INVALID: workflow is not accepting lane artifacts", 2);
  if (scope.startsWith("lane:")) {
    const laneId = scope.slice(5);
    const lane = receipt.lanes.find((candidate) => candidate.id === laneId);
    if (!lane) throw new FriendlyAdversaryError("FA_PLAN_MISMATCH: capability lane is not in the immutable plan", 2);
...
export async function preflightWorkflowArtifact(input: { workflow?: WorkflowKind; authorityId: string; capability: string; relativePath: string }): Promise<{ run_id: string; relative_path: string; scope: string }> {
  const relativePath = validateRelativeArtifact(input.relativePath);
  const { receipt, scope } = await validateAuthority(input.authorityId, input.capability, relativePath);
  if (input.workflow && receipt.kind !== input.workflow) throw new FriendlyAdversaryError("FA_WORKFLOW_MISMATCH: authority belongs to another workflow", 2);
...
async function laneCompleted(runDirectory: string, lane: WorkflowLane): Promise<boolean> {
  const target = lanePath(lane);
  if (lane.kind !== "decision") return publicationExists(runDirectory, `lane:${lane.id}`, target);
  ...
  for (const entry of entries.filter((name) => new RegExp(`^decision-${lane.id}-[0-9]{4}\\.md$`, "u").test(name))) {
    if (await publicationExists(runDirectory, `lane:${lane.id}`, entry)) return true;
  }
  return false;
}

async function nextDecisionPath(runDirectory: string, lane: WorkflowLane): Promise<string> {
  const entries = await readdir(runDirectory).catch(() => []);
  let highest = 0;
  for (const entry of entries) {
    const match = new RegExp(`^decision-${lane.id}-([0-9]{4})\\.md$`, "u").exec(entry);
    if (match?.[1] && await publicationExists(runDirectory, `lane:${lane.id}`, entry)) highest = Math.max(highest, Number(match[1]));
  }
  if (highest >= 9999) throw new FriendlyAdversaryError(`FA_DECISION_REVISION_EXHAUSTED: ${lane.id}`, 2);
  return `decision-${lane.id}-${String(highest + 1).padStart(4, "0")}.md`;
}

async function reconcilePendingDecisionRevisions(receipt: WorkflowReceipt): Promise<void> {
  let changed = false;
...
export async function completeWorkflow(input: { workflow?: WorkflowKind; authorityId: string; capability: string; artifacts: Array<{ relativePath: string; markdown: string }>; userSignoff?: boolean; signal?: AbortSignal }): Promise<{ run_id: string; artifacts: Array<{ relative_path: string; sha256: string }> }> {
  if (!input.artifacts.length) throw new FriendlyAdversaryError("FA_OUTCOME_INVALID: final outcome requires artifacts", 2);
  ...
  if (input.workflow && receipt.kind !== input.workflow) throw new FriendlyAdversaryError("FA_WORKFLOW_MISMATCH: authority belongs to another workflow", 2);
  if (scope !== "outcome") throw new FriendlyAdversaryError("FA_CAPABILITY_DENIED: outcome capability required", 2);
  if (!["planned", "incomplete", "ready"].includes(receipt.status)) throw new FriendlyAdversaryError("FA_RUN_STATE_INVALID: workflow is not accepting a final outcome", 2);
  if (Object.keys(receipt.pendingDecisionRevisions).length) throw new FriendlyAdversaryError("FA_REVISION_PENDING: every requested decision revision must publish before completion", 2);
  const missing: string[] = [];
  for (const lane of receipt.lanes) if (!await laneCompleted(receipt.outputDirectory, lane)) missing.push(lane.id);
  if (missing.length) throw new FriendlyAdversaryError(`FA_LANES_INCOMPLETE: missing workflow lanes: ${missing.join(", ")}`, 2);
  if (receipt.kind === "design-new-codebase" && input.userSignoff !== true) throw new FriendlyAdversaryError("FA_SIGNOFF_REQUIRED: explicit user signoff is required", 2);
  ...
  const results: Array<{ relative_path: string; sha256: string }> = [];
  ...
  if (receipt.kind === "audit-codebase") {
    ...
    normalizedArtifacts.set(
      "adjudication.md",
      markdownEnvelope(`${adjudication.markdown.trimEnd()}\n\n<!-- ${OUTCOME_REPORT_SHA256}:${sha256(report)} -->`),
    );
  }
...
async function walk(root: string, relative = ""): Promise<string[]> {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    else throw new FriendlyAdversaryError("FA_ARTIFACT_UNSAFE: workflow artifacts contain an unsupported filesystem entry", 2);
  }
  return files;
}
...
async function expectedManifest(receipt: WorkflowReceipt, overrides = new Map<string, Buffer>()): Promise<string> {
  const files = (await walk(receipt.outputDirectory)).filter((file) => file !== MANIFEST);
  const lines: string[] = [];
  for (const file of files) lines.push(manifestLine(file, overrides.get(file) ?? await readFile(path.join(receipt.outputDirectory, ...file.split("/")))));
  return manifestEnvelope(lines.join("\n"));
}

async function validateAuditOutcomeBinding(receipt: WorkflowReceipt): Promise<void> {
  if (receipt.kind !== "audit-codebase") return;
...
export async function sealWorkflow(runDirectory: string): Promise<WorkflowReceipt> {
  ...
  if (receipt.status === "sealed" || receipt.status === "sealed-incomplete") {
    try {
      ...
      if (!manifest || committedSlotContent(MANIFEST, manifest) || !(await sealTransactionMatches(receipt, expected))) throw error;
  ...
  if (receipt.status !== "ready") throw new FriendlyAdversaryError(`FA_SEAL_STATE_INVALID: cannot seal workflow in ${receipt.status}`, 2);
  if (Object.keys(receipt.pendingDecisionRevisions).length) throw new FriendlyAdversaryError("FA_REVISION_PENDING: a workflow with pending decision revisions cannot seal", 2);
...
export async function verifyWorkflow(runDirectory: string): Promise<{ valid: true; artifacts: number }> {
  const receipt = await readReceipt(runDirectory);
  if (receipt.status !== "sealed" && receipt.status !== "sealed-incomplete") throw new FriendlyAdversaryError("FA_VERIFY_STATE_INVALID: workflow is not sealed", 2);
  ...
  const expected = new Map<string, { hash: string; bytes: number }>();
  for (const line of lines) {
    const match = /^([a-f0-9]{64})  (\d+)  (".*")$/u.exec(line);
    if (!match) throw new FriendlyAdversaryError("FA_MANIFEST_INVALID: artifact manifest is malformed", 2);
    expected.set(JSON.parse(match[3]!) as string, { hash: match[1]!, bytes: Number(match[2]) });
  }
  const actual = (await walk(receipt.outputDirectory)).filter((file) => file !== MANIFEST);
  if (actual.length !== expected.size || actual.some((file) => !expected.has(file))) throw new FriendlyAdversaryError("FA_MANIFEST_MISMATCH: artifact inventory differs from the seal", 3);
  for (const file of actual) {
    const content = await readFile(path.join(receipt.outputDirectory, ...file.split("/")));
    const record = expected.get(file)!;
    if (content.byteLength !== record.bytes || sha256(content) !== record.hash) throw new FriendlyAdversaryError(`FA_ARTIFACT_TAMPERED: ${file}`, 3);
  }
  return { valid: true, artifacts: actual.length + 1 };
}

async function revokeWorkflowAuthorities(runDirectory: string, preserveAuthorityId?: string): Promise<void> {
  ...
  for (const entry of entries.filter((name) => name.startsWith(prefix) && AUTHORITY_ID.test(name.slice(0, 32)) && name.endsWith(".workflow.json"))) {
    ...
    if (authorityId === preserveAuthorityId) continue;
    await rm(path.join(root, entry), { force: true });
  }
}

export async function resumeWorkflow(runDirectory: string, revisionLaneIds: string[] = []): Promise<WorkflowAuthorityPacket> {
  return withWorkflowMutation(runDirectory, "workflow-resume", async (receipt) => {
  if (receipt.status !== "planned" && receipt.status !== "incomplete") throw new FriendlyAdversaryError("FA_RESUME_STATE_INVALID: only planned or incomplete workflows may resume", 2);
  await reconcilePendingDecisionRevisions(receipt);
  const current = await currentSnapshot(receipt);
  if (current.digest !== receipt.snapshot.digest) throw new FriendlyAdversaryError("FA_SNAPSHOT_STALE: changed snapshot requires a new workflow run", 3);
  const scopes: Record<string, { exactPaths?: string[]; prefixPaths?: string[]; maxArtifacts?: number }> = {};
  ...
  if (revisionLaneIds.length && receipt.kind !== "design-new-codebase") throw new FriendlyAdversaryError("FA_REVISION_INVALID: only design workflows support decision revisions", 2);
  const requestedRevisions = new Set(revisionLaneIds);
  if (requestedRevisions.size !== revisionLaneIds.length || revisionLaneIds.some((id) => !SAFE_ID.test(id))) throw new FriendlyAdversaryError("FA_REVISION_INVALID: decision revision lane IDs are malformed or duplicated", 2);
  ...
  if (unavailableRevisions.length) throw new FriendlyAdversaryError(`FA_REVISION_INVALID: unknown decision lanes: ${unavailableRevisions.join(", ")}`, 2);
  ...
  for (const lane of eligible) {
    if (await laneCompleted(receipt.outputDirectory, lane) && !revisionsToIssue.has(lane.id)) continue;
  ...
  if (!Object.keys(scopes).length) throw new FriendlyAdversaryError("FA_RESUME_STATE_INVALID: workflow has no eligible incomplete phase", 2);
  const replacement = await createAuthority(receipt, scopes);
  await revokeWorkflowAuthorities(receipt.outputDirectory, replacement.authority_id);
  return replacement;
  });
...
export async function abortWorkflow(runDirectory: string): Promise<WorkflowReceipt> {
  return withWorkflowMutation(runDirectory, "workflow-abort", async (receipt) => {
  if (receipt.status === "sealed" || receipt.status === "sealed-incomplete") throw new FriendlyAdversaryError("FA_ABORT_SEALED: sealed workflows cannot be aborted", 2);
...
export async function workflowStatus(runDirectory: string): Promise<WorkflowReceipt> {
