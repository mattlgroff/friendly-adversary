...
type WasiValue = number | bigint;

export type WasiPreview1Function = (...args: WasiValue[]) => number;
export type WasiPreview1Imports = Record<string, WasiPreview1Function>;
...
function numberArgument(value: WasiValue | undefined): number | undefined {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 0 ? value : undefined;
}

function bigintArgument(value: WasiValue | undefined): bigint | undefined {
  if (typeof value === "bigint" && value >= 0n) return value;
  if (typeof value === "number" && Number.isSafeInteger(value) && value >= 0) return BigInt(value);
  return undefined;
}

function memoryRegion(memory: WebAssembly.Memory, offset: number, length: number): Uint8Array | undefined {
  if (!Number.isSafeInteger(offset) || !Number.isSafeInteger(length) || offset < 0 || length < 0) return undefined;
  if (offset > memory.buffer.byteLength || length > memory.buffer.byteLength - offset) return undefined;
  return new Uint8Array(memory.buffer, offset, length);
}

function memoryView(memory: WebAssembly.Memory, offset: number, length: number): DataView | undefined {
...
function resolveGuestPath(root: string, parent: string, guestPath: string): string | undefined {
  if (guestPath.includes("\0") || path.posix.isAbsolute(guestPath) || path.win32.isAbsolute(guestPath)) return undefined;
...
function directoryDescriptor(candidate: string, rights: bigint): DirectoryDescriptor | undefined {
  const resolved = realpathSync.native(candidate);
...
function filesystemErrno(error: unknown): number {
  const code = (error as NodeJS.ErrnoException).code;
  if (code === "EACCES") return ERRNO.access;
  if (code === "EPERM") return ERRNO.permission;
  if (code === "ENOENT") return ERRNO.noEntry;
  if (code === "ENOTDIR") return ERRNO.notDirectory;
  if (code === "EOVERFLOW") return ERRNO.overflow;
  return ERRNO.io;
}

function directoryEntryType(entry: ReturnType<typeof readdirSync>[number]): number {
  ...
  if (entry.isBlockDevice()) return FILETYPE.blockDevice;
  if (entry.isCharacterDevice()) return FILETYPE.characterDevice;
  if (entry.isSocket()) return FILETYPE.socketStream;
  return FILETYPE.unknown;
}

function requiredImport(imports: WasiPreview1Imports, name: string): WasiPreview1Function {
  const implementation = imports[name];
  if (!implementation) throw new Error(`Node WASI is missing required import: ${name}`);
  return implementation;
}

export function withWindowsDirectoryReads(
  nativeImports: WasiPreview1Imports,
  repository: string,
  ...
  if (!rootDescriptor) throw new Error(`WASI preopen is not a directory: ${repository}`);
  const directories = new Map<number, DirectoryDescriptor>([
    [PREOPEN_DIRECTORY_FD, rootDescriptor],
  ]);

  const wrapped: WasiPreview1Imports = { ...nativeImports };

  wrapped.path_open = (...args: WasiValue[]): number => {
    const result = pathOpen(...args);
    if (result !== ERRNO.success) return result;
    ...
    if (!pathBytes || !resultView) return result;
    ...
    try {
      ...
      if (descriptor) directories.set(openedFd, descriptor);
    } catch {
  ...
  wrapped.fd_close = (...args: WasiValue[]): number => {
    const result = fdClose(...args);
    const fd = numberArgument(args[0]);
    if (result === ERRNO.success && fd !== undefined) directories.delete(fd);
    return result;
  };

  const fdRenumber = nativeImports.fd_renumber;
  if (fdRenumber) {
    wrapped.fd_renumber = (...args: WasiValue[]): number => {
      ...
      if (result === ERRNO.success && from !== undefined && to !== undefined) {
        ...
        if (directory) directories.set(to, directory);
      }
      return result;
    };
  }

  wrapped.fd_readdir = (...args: WasiValue[]): number => {
    const fd = numberArgument(args[0]);
    ...
    if (cookie > BigInt(Number.MAX_SAFE_INTEGER)) return ERRNO.overflow;

    const memory = getMemory();
    const output = memoryRegion(memory, bufferPointer, bufferLength);
    const usedView = memoryView(memory, usedPointer, 4);
    if (!output || !usedView) return ERRNO.fault;
    usedView.setUint32(0, 0, true);

    const directory = directories.get(fd);
    if (!directory) return ERRNO.badDescriptor;
    if ((directory.rights & RIGHT_FD_READDIR) === 0n) return ERRNO.notCapable;
