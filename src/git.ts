...
export interface GitContext {
  ...
  artifacts: {
    ...
    combinedDiff: Buffer;
    indexDiff: Buffer;
    workingTreeDiff: Buffer;
    zeroContextDiff: Buffer;
    diffStat: Buffer;
    diffNumstat: Buffer;
    recentCommits: Buffer;
    untracked: Map<string, { kind: "file" | "symlink"; content: Buffer }>;
  };
}
...
function parseNullList(content: Buffer): string[] {
  return content.toString("utf8").split("\0").filter(Boolean);
}

export function ignoredGitPaths(repo: string, files: string[]): Set<string> {
...
export function monitoredGitPaths(repo: string): string[] {
...
export function monitoredGitDirectories(repo: string): string[] {
  const directories = new Set<string>(["."]);
  const addAncestors = (initial: string): void => {
  ...
  for (const candidate of monitoredGitPaths(repo)) addAncestors(path.posix.dirname(candidate));
  for (const candidate of parseNullList(gitBuffer(repo, ["ls-files", "--others", "--directory", "--exclude-standard", "-z"]))) {
    ...
    if (directory && !inOutputDirectory(directory)) addAncestors(directory);
  }
  return [...directories].sort();
}

function inOutputDirectory(file: string): boolean {
  return file === OUTPUT_DIRECTORY || file.startsWith(`${OUTPUT_DIRECTORY}/`);
}

const REVIEW_SCOPE = [".", `:(exclude)${OUTPUT_DIRECTORY}/**`];

export async function resolveRepositoryRoot(candidate: string): Promise<string> {
...
export function resolveBaseRef(repo: string, explicitBase?: string): string {
  if (explicitBase) {
    if (!refExists(repo, explicitBase)) {
      throw new FriendlyAdversaryError(
        `The requested base ref does not resolve to a commit: ${explicitBase}`,
        2,
      );
    }
    return explicitBase;
  ...
  try {
    ...
    if (symbolic.startsWith("refs/remotes/")) {
      ...
      if (refExists(repo, remoteRef)) return remoteRef;
    }
  } catch {
  ...
  throw new FriendlyAdversaryError(
    "No review baseline was found. Pass --base with a commit or branch from the existing application.",
    2,
  );
}
...
async function untrackedLines(repo: string, file: string): Promise<number[]> {
  const absolute = path.join(repo, file);
  const metadata = await lstat(absolute);
  if (metadata.isSymbolicLink()) return [1];
...
async function changedLineMap(
  ...
  for (const file of untracked) result[file] = await untrackedLines(repo, file);
  return result;
}
...
function hashSnapshot(
  ...
  changedFiles: string[],
  changedLines: Record<string, number[]>,
  artifacts: GitContext["artifacts"],
): string {
  const hash = createHash("sha256");
  const part = (label: string, content: string | Buffer): void => {
...
export async function inspectGit(
  ...
  const untracked = new Map<string, { kind: "file" | "symlink"; content: Buffer }>();
  for (const file of untrackedFiles) {
    const absolute = path.join(repositoryRoot, file);
    const metadata = await lstat(absolute);
    if (metadata.isSymbolicLink()) untracked.set(file, { kind: "symlink", content: Buffer.from(await readlink(absolute)) });
    else if (metadata.isFile()) untracked.set(file, { kind: "file", content: await readFile(absolute) });
  }
  const artifacts: GitContext["artifacts"] = {
...
function safeArtifactPath(root: string, repositoryPath: string): string {
...
export async function writeGitArtifacts(
  context: GitContext,
  runDirectory: string,
): Promise<void> {
  ...
  await writeFileAtomic(
    path.join(gitDirectory, "untracked-files.txt"),
  ...
  for (const file of context.untrackedFiles) {
    ...
    if (!captured || captured.kind !== "file") continue;
  ...
  await writeFileAtomic(
    path.join(gitDirectory, "trusted-instructions.txt"),
    `${instructionFiles.join("\n")}${instructionFiles.length ? "\n" : ""}`,
  );
}

export async function snapshotStillMatches(context: GitContext): Promise<boolean> {
  if (!await pathExists(context.repositoryRoot)) return false;
