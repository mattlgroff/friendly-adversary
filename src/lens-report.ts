...
export type Publication = "created" | "confirmed_existing";
export type ToolOperation = "publish";
...
function markdownOutsideHtmlComments(content: string): string {
  let visible = "";
  let cursor = 0;
  while (cursor < content.length) {
    const opening = content.indexOf("<!--", cursor);
    if (opening < 0) return visible + content.slice(cursor);
...
export function validateLensReportContract(content: string, file: string): void {
...
export function validatePersistedLensReportContract(content: string, file: string): void {
  const envelope = /^- Model:\s*[^\r\n]+\r?\n- Effort:\s*[^\r\n]+\r?\n- Host:\s*[^\r\n]+\r?\n\r?\n/u.exec(content);
  if (!envelope) fail("FA_LENS_REPORT_INVALID", `${file} must record '- Model: <value>', '- Effort: <value>', and '- Host: <value>' in its tool-owned metadata envelope`);
  const markerIndex = content.lastIndexOf(LENS_COMPLETE);
  if (markerIndex < envelope[0].length) fail("FA_LENS_REPORT_INCOMPLETE", `${file} lacks its lens completion marker`);
  validateLensReportContract(content.slice(envelope[0].length, markerIndex).trim(), file);
}

function throwIfCancelled(signal?: AbortSignal): void {
  if (signal?.aborted) fail("FA_CANCELLED", "tool call was cancelled before publication", 3);
}

function validateBody(body: string, name: string, maximumBytes: number, allowedControlMarkers: string[] = []): string {
  const normalized = body.trim();
  if (!normalized) fail("FA_REPORT_CONTRACT_INVALID", `${name} must not be empty`);
  ...
  for (const marker of allowedControlMarkers) {
    ...
    if (occurrences > 1) duplicateAllowedMarker = true;
  ...
  if (secret) fail("FA_SECRET_PATTERN_DETECTED", `${name} contains a recognizable ${secret} value`);
  return normalized;
}

async function requireSafeDirectory(directory: string, code: string): Promise<string> {
  const metadata = await lstat(directory).catch(() => fail(code, "required directory is missing"));
  if (metadata.isSymbolicLink() || !metadata.isDirectory()) fail(code, "required directory is redirected or not a directory");
  return realpath(directory);
}

async function requireSingleRegularFile(filePath: string, code: string): Promise<void> {
...
async function validateRun(plan: RunPlan, runDirectory: string): Promise<ReviewReceipt> {
  ...
  if (runReal !== plan.outputDirectory) fail("FA_PATH_OUTSIDE_RUN", "authorized run path differs from its private plan");
  await requireSingleRegularFile(path.join(runReal, RECEIPT_FILE), "FA_STATE_INVALID");
  const receipt = await readJson<ReviewReceipt>(path.join(runReal, RECEIPT_FILE)).catch(() => fail("FA_STATE_INVALID", "run state is malformed"));
  if (
    receipt.schemaVersion !== "1" || receipt.runId !== plan.runId || receipt.repositoryRoot !== plan.repositoryRoot
...
async function publishNoReplace(
  ...
  if (existing) {
    if (existing.equals(Buffer.from(content))) return "confirmed_existing";
  ...
  if (parent !== path.dirname(target)) fail("FA_PUBLICATION_PARENT_CHANGED", "artifact parent was redirected");
  const scratch = await requireSafeDirectory(`${runDirectory}.scratch`, "FA_PUBLICATION_SCRATCH_CHANGED");
  if (path.dirname(scratch) !== path.dirname(runDirectory)) fail("FA_PATH_OUTSIDE_RUN", "publication scratch escaped the private run namespace");
  ...
  try {
    ...
    if (!raced.equals(Buffer.from(content))) fail("FA_REPORT_ALREADY_DIFFERENT", "artifact already contains a different committed publication");
...
async function withAuthorizedRun<T extends object>(input: {
  authorityId: string;
  writeCapability: string;
  scope: CapabilityScope;
  signal?: AbortSignal;
}, callback: (context: { plan: RunPlan; runDirectory: string; receipt: ReviewReceipt }) => Promise<T>): Promise<T> {
...
function lensEnvelope(plan: RunPlan, body: string): string {
  return `- Model: ${plan.lensModel}\n- Effort: ${plan.lensEffort}\n- Host: ${plan.lensHost}\n\n${body}\n\n${LENS_COMPLETE}\n`;
}

function adjudicationEnvelope(plan: RunPlan, body: string, reportSha256: string): string {
  return `- Model: ${plan.plannedModel}\n- Effort: ${plan.plannedEffort}\n- Host: ${plan.host}\n\n${body}\n\n<!-- ${OUTCOME_REPORT_SHA256}:${reportSha256} -->\n${DOCUMENT_COMPLETE}\n`;
}

function reportEnvelope(body: string): string {
  return `${body}\n\n${DOCUMENT_COMPLETE}\n`;
}

export interface RecordLensReportInput {
...
export interface RecordLensReportResult {
  ok: true;
  runId: string;
  operation: ToolOperation;
...
export async function recordLensReport(
  input: RecordLensReportInput,
): Promise<RecordLensReportResult> {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(input.lensId)) fail("FA_LENS_INVALID", "lens_id is malformed");
  if (input.reportMarkdown === undefined) {
    fail("FA_REPORT_REQUIRED", "publish requires report_markdown");
  }
  const scope: CapabilityScope = { operation: "lens", lensId: input.lensId };
  return withAuthorizedRun({
    ...
    if (!plan.expectedLenses.includes(input.lensId)) fail("FA_LENS_NOT_EXPECTED", "lens is not selected for this run");
    const lensesRoot = await requireSafeDirectory(path.join(runDirectory, "lenses"), "FA_LENS_DIRECTORY_INVALID");
    if (path.dirname(lensesRoot) !== runDirectory) fail("FA_PATH_OUTSIDE_RUN", "lens directory escapes its run");
    ...
    if (Buffer.byteLength(content) > MAX_LENS_REPORT_BYTES) fail("FA_REPORT_TOO_LARGE", "lens report envelope exceeds the byte limit");
...
interface OutcomeArtifactReceipt {
...
export interface RecordReviewOutcomeResult {
  ...
  artifacts?: OutcomeArtifactReceipt[];
  publication?: Publication;
  nextAction: string;
}

export async function validateCommittedOutcome(runDirectory: string): Promise<void> {
  const runReal = await requireSafeDirectory(runDirectory, "FA_RUN_NOT_FOUND");
  for (const file of ["adjudication.md", "report.md"]) await requireSingleRegularFile(path.join(runReal, file), "FA_OUTCOME_INCOMPLETE");
...
async function requireCompleteLenses(plan: RunPlan, runDirectory: string): Promise<void> {
  const lensesRoot = await requireSafeDirectory(path.join(runDirectory, "lenses"), "FA_LENS_DIRECTORY_INVALID");
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) fail("FA_LENSES_INCOMPLETE", "not every selected lens has published exactly one report");
  for (const file of actual) {
    ...
    if (!content.length) fail("FA_LENSES_INCOMPLETE", "not every selected lens has published exactly one report");
    if (!content.trimEnd().endsWith(LENS_COMPLETE)) fail("FA_LENS_REPORT_INCOMPLETE", "a lens report lacks its completion marker");
    const metadata = `- Model: ${plan.lensModel}\n- Effort: ${plan.lensEffort}\n- Host: ${plan.lensHost}\n\n`;
    if (!content.startsWith(metadata)) fail("FA_LENS_REPORT_INVALID", "a lens report has an invalid tool-owned metadata envelope");
...
export async function recordReviewOutcome(
  ...
  return withAuthorizedRun({
    ...
    const artifacts: OutcomeArtifactReceipt[] = [
