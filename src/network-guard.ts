...
function replace(moduleName: string, names: string[]): void {
  ...
  for (const name of names) {
    if (name in module) module[name] = denyNetwork;
  }
}
...
export function disableNetworkAccess(): void {
  const net = require("node:net") as MutableModule & { Socket?: { prototype?: MutableModule } };
  replace("node:net", ["connect", "createConnection"]);
  if (net.Socket?.prototype) {
    net.Socket.prototype.connect = denyNetwork;
    net.Socket.prototype.open = denyNetwork;
  }
  const tls = require("node:tls") as MutableModule & { TLSSocket?: { prototype?: MutableModule } };
