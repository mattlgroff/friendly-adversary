...
import { withWindowsDirectoryReads } from "./wasi-preview1-windows.js";
...
function parse(argv) {
    ...
    const unsupported = ripgrepArgs.find((argument) => (argument === "--threads"
    ...
    if (unsupported)
        return fail(`Unsupported ripgrep WebAssembly argument: ${unsupported}`);
...
async function main() {
    ...
    if (!(await lstat(repository)).isDirectory())
        fail(`Repository is not a directory: ${parsed.repo}`);
    ...
    if (!(exportedMemory instanceof WebAssembly.Memory))
        fail("ripgrep WebAssembly does not export memory");
...
//# sourceMappingURL=ripgrep-wasm-cli.js.map