...
import { LENS_HOST, LENS_MODEL, LENS_REASONING_EFFORT, LENS_SERVICE_TIER, RECEIPT_FILE, } from "./constants.js";
import { FriendlyAdversaryError } from "./errors.js";
import { writeFileAtomic, writeJsonAtomic } from "./fs-utils.js";
import { MAX_LENS_REPORT_BYTES, recordLensReport } from "./lens-report.js";
...
const activePublicationControllers = new Set();
...
function lensFailureCategory(reason) {
    const message = reason instanceof Error ? reason.message : String(reason);
    const normalized = message.toLowerCase();
    if (message.includes("FA_CODEX_UNAVAILABLE"))
        return "codex_unavailable";
    if (message.includes("FA_CODEX_LENS_TIMEOUT"))
        return "lens_timeout";
    if (message.includes("FA_CANCELLED") || message.includes("Review cancelled"))
        return "cancelled";
    if (/unauthorized|authentication|authenticate|log in|login|\b401\b/u.test(normalized))
        return "authentication_failed";
    if (/rate.?limit|quota|\b429\b/u.test(normalized))
        return "quota_or_rate_limited";
    if (/model.*(?:unavailable|unsupported|not found)|(?:unavailable|unsupported).*model/u.test(normalized))
        return "model_unavailable";
    if (/network|connect|dns|tls|certificate/u.test(normalized))
        return "network_failed";
    if (message.includes("FA_REPORT_") || message.includes("FA_LENS_") || message.includes("FA_DOCUMENT_"))
        return "invalid_report";
    if (message.includes("FA_CAPABILITY_") || message.includes("FA_AUTHORITY_") || message.includes("FA_PUBLICATION_"))
        return "publication_failed";
    return "lens_execution_failed";
}
function retainSignalHandling() {
...
function handleShutdown(signal) {
    ...
    for (const controller of activePublicationControllers)
        controller.abort();
...
function appendBounded(chunks, chunk) {
    ...
    if (current >= MAX_DIAGNOSTIC_BYTES)
        return;
    chunks.push(chunk.subarray(0, MAX_DIAGNOSTIC_BYTES - current));
}
export function runLensProcess(executable, args, cwd, input, timeoutMs) {
    return new Promise((resolve, reject) => {
        ...
        child.once("close", (code, signal) => {
            ...
            if (timedOut)
                return reject(new FriendlyAdversaryError(`FA_CODEX_LENS_TIMEOUT: codex exceeded ${timeoutMs}ms`, 3));
            if (code === 0)
                resolve(output);
            else
                reject(new FriendlyAdversaryError(`FA_CODEX_LENS_FAILED: codex exited ${code ?? signal ?? "unknown"}${output.stderr.trim() ? `: ${output.stderr.trim()}` : ""}`, 3));
        });
        child.stdin.end(input);
    });
}
export function createLocalCodexRuntime(executable = "codex", prefixArgs = []) {
    return {
        async version() {
            const result = await runLensProcess(executable, [...prefixArgs, "--version"], process.cwd(), undefined, CODEX_VERSION_TIMEOUT_MS);
            const version = result.stdout.trim();
            if (!version)
                throw new FriendlyAdversaryError("FA_CODEX_UNAVAILABLE: codex --version returned no version", 3);
            return version;
        },
...
export const localCodexRuntime = createLocalCodexRuntime();
function lensPrompt(lensId, receipt) {
...
function invocation(lensId, receipt) {
...
export async function runCodexLenses(receipt, authority, runtime = localCodexRuntime, verifiedCodexVersion) {
    ...
    try {
        ...
        const executions = receipt.expectedLenses.map(async (lensId) => {
            ...
            try {
                ...
                if (!writeCapability)
                    throw new FriendlyAdversaryError(`FA_CAPABILITY_MISSING: no capability for ${lensId}`, 3);
        ...
        if (failures.length) {
            ...
            throw new FriendlyAdversaryError(`${batchShutdownSignal ? `Review cancelled by ${batchShutdownSignal}` : "FA_CODEX_LENSES_INCOMPLETE"}. Run: ${receipt.outputDirectory}. ${reason}`, signalExitCode ?? 3);
        }
...
//# sourceMappingURL=lens-runner.js.map