...
async function defaultRuntimeDirectory() {
    const candidates = [
...
export async function runRuffWasm(options) {
    const major = Number.parseInt(process.versions.node.split(".")[0] ?? "0", 10);
    if (major < 22)
        throw new FriendlyAdversaryError(`Ruff WebAssembly requires Node 22 or newer, received ${process.version}`, 2);
    ...
    if (options.signal?.aborted)
        throw new FriendlyAdversaryError("Ruff WebAssembly review was cancelled", 130);
    const files = [...options.files].sort(comparePaths);
    const seen = new Set();
    let totalBytes = 0;
    for (const file of files) {
        if (seen.has(file.path))
            throw new FriendlyAdversaryError(`Duplicate Ruff input path: ${file.path}`, 2);
        seen.add(file.path);
        const bytes = Buffer.byteLength(file.source, "utf8");
        totalBytes += bytes;
        if (bytes > maxFileBytes)
            throw new FriendlyAdversaryError(`Ruff input ${file.path} exceeds the ${maxFileBytes}-byte file limit`, 2);
    }
    if (totalBytes > maxTotalBytes)
        throw new FriendlyAdversaryError(`Ruff inputs exceed the ${maxTotalBytes}-byte batch limit`, 2);
    ...
    const response = await new Promise((resolve, reject) => {
        ...
        worker.once("exit", (code) => {
            if (code !== 0)
                finish(() => reject(new FriendlyAdversaryError(`Ruff WebAssembly worker exited with code ${code}`, 2)));
        });
    });
    await worker.terminate();
    if (!response.ok)
        throw new FriendlyAdversaryError(`Ruff WebAssembly failed: ${response.error}`, 2);
    ...
    if (response.results.length !== files.length)
        throw new FriendlyAdversaryError("Ruff WebAssembly returned the wrong number of file results", 2);
    return {
...
//# sourceMappingURL=ruff-wasm.js.map