...
export function planProductVersionIsCompatible(value) {
    return value === PRODUCT_VERSION;
}
function lengthDelimitedHash(domain, fields) {
...
async function ensurePrivateDirectory(parent, directory) {
    ...
    const unsafePosixPermissions = process.platform !== "win32" && ((currentUid !== undefined && metadata.uid !== BigInt(currentUid))
        || (metadata.mode & 448n) !== 448n
...
export async function ensureAuthorityRoot() {
...
async function authorityRootIdentity(root) {
...
async function assertAuthorityRootIdentity(root, expected) {
...
export async function cleanupExpiredAuthorities(now = new Date()) {
    ...
    for (const entry of entries) {
        if (!entry.isFile() || !/^[a-f0-9]{32}(?:\.workflow)?\.json$/u.test(entry.name))
            continue;
...
function authorityPath(authorityId) {
...
async function readStableSingleRegularFile(filePath, missingCode, unsafeCode) {
    ...
    try {
        ...
        if (authorityDirectory && authorityIdentity)
            await assertAuthorityRootIdentity(authorityDirectory, authorityIdentity);
        return content;
    }
    finally {
        await handle.close();
...
export async function readAuthorityControlFile(filePath, missingCode = "FA_AUTHORITY_NOT_FOUND", unsafeCode = "FA_AUTHORITY_UNSAFE") {
    const root = await ensureAuthorityRoot();
...
async function publishExclusive(filePath, content, beforePublish) {
    const authorityDirectory = path.dirname(filePath) === authorityRoot() ? await ensureAuthorityRoot() : undefined;
    const authorityIdentity = authorityDirectory ? await authorityRootIdentity(authorityDirectory) : undefined;
    ...
    try {
        ...
        if (authorityDirectory && authorityIdentity)
            await assertAuthorityRootIdentity(authorityDirectory, authorityIdentity);
        await beforePublish?.();
        await link(candidatePath, filePath).catch((error) => {
            if (error.code === "EEXIST")
                throw new FriendlyAdversaryError("FA_AUTHORITY_EXISTS: authority identifier collision", 3);
            throw error;
        });
        ...
        if (!opened.isFile() || opened.nlink !== 2n
...
export async function publishAuthorityControlFile(filePath, content, options = {}) {
    const root = await ensureAuthorityRoot();
    if (path.dirname(filePath) !== root || !/^[a-f0-9]{32}(?:\.workflow)?\.json$/u.test(path.basename(filePath))) {
...
export async function createReviewAuthority(input) {
    ...
    for (const lensId of plan.expectedLenses) {
        ...
        const scope = { operation: "lens", lensId };
    ...
    const authority = { ...authorityBase, capabilities: digests };
    try {
...
function validatePlan(value) {
    ...
    if (!plan || plan.schemaVersion !== "1" || !planProductVersionIsCompatible(plan.productVersion)
...
function validateAuthorityRecord(value, authorityId) {
    const record = value;
    if (!record || record.schemaVersion !== AUTHORITY_SCHEMA_VERSION || record.authorityId !== authorityId
...
export async function validateReviewAuthority(input) {
    ...
    if (!safeEqualHex(sha256(planContent), authority.planSha256)
...
export async function liveReviewAuthorityId(runDirectoryInput, now = new Date()) {
    ...
    try {
        authorityContent = await readStableSingleRegularFile(authorityPath(plan.authorityId), "FA_AUTHORITY_NOT_FOUND", "FA_AUTHORITY_UNSAFE");
    }
    catch (error) {
        if (error instanceof FriendlyAdversaryError && error.message.startsWith("FA_AUTHORITY_NOT_FOUND:"))
            return undefined;
        throw error;
    }
...
export async function hasLiveReviewAuthority(runDirectoryInput, now = new Date()) {
    return await liveReviewAuthorityId(runDirectoryInput, now) !== undefined;
}
export async function revokeReviewAuthorityById(authorityId) {
