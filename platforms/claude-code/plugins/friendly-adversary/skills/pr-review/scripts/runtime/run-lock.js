...
export function classifyProcessIdentity(alive, processStartedAt, currentIdentity) {
    if (!alive)
        return "dead";
    if (!currentIdentity || processStartedAt === "unavailable")
        return "unsupported-live-process";
    return processStartedAt === currentIdentity ? "exact-owner" : "pid-reused";
}
function processIdentity(pid, processStartedAt) {
    if (pid === process.pid) {
...
export function runLockPath(repositoryRoot, runReal) {
    return path.join(repositoryRoot, ".friendly-adversary", ".internal", `lifecycle-${path.basename(runReal)}.lock`);
}
function parseRecord(content) {
    try {
        const record = JSON.parse(content);
        if (record.schemaVersion === "1" && Number.isSafeInteger(record.pid) && Number(record.pid) > 0
...
async function acquireRecoveryClaim(claimPath) {
    const file = await open(claimPath, "wx", 0o600).catch((error) => {
        if (error.code === "EEXIST") {
            throw new FriendlyAdversaryError(`FA_LOCK_RECOVERY_BUSY: recovery claim exists at ${claimPath}. Confirm no recovery process is active before removing only this claim.`, 3);
        }
        throw error;
    });
...
async function releaseRecoveryClaim(claim, remove = unlink) {
    let owned = false;
    try {
...
export async function acquireRunLock(repositoryRoot, runReal, operation) {
    ...
    try {
        ...
        if (record && identity === "unsupported-live-process") {
            throw new FriendlyAdversaryError(`FA_PROCESS_IDENTITY_UNAVAILABLE: PID ${record.pid} is alive, but exact start-time identity is unavailable for an external process; recovery requires operator inspection`, 3);
        }
            ...
            for (let attempt = 0;; attempt += 1) {
                try {
                    ...
                    if (!TRANSIENT_RELEASE_ERRORS.has(code) || attempt >= 11) {
                        throw new FriendlyAdversaryError("FA_RUN_LOCK_RELEASE: lifecycle lock cleanup failed after the operation. Recover the run lock before retrying.", 3);
                    }
...
export async function recoverRunLock(repositoryRoot, runReal, hooks = {}) {
    const lockPath = runLockPath(repositoryRoot, runReal);
    ...
    if (!initialIdentity)
        throw new FriendlyAdversaryError("No run lock exists", 2);
    if (!initialRecord)
        throw new FriendlyAdversaryError("Run lock is malformed; recovery is ambiguous and fails closed", 3);
    const claim = await acquireRecoveryClaim(`${lockPath}.recovering-${initialRecord.ownerNonce}`);
    let failure;
    try {
        ...
        if (!currentIdentity)
            throw new FriendlyAdversaryError("No run lock exists", 2);
        if (currentIdentity.device !== initialIdentity.device || currentIdentity.inode !== initialIdentity.inode
        ...
        if (identity === "unsupported-live-process") {
            throw new FriendlyAdversaryError(`FA_PROCESS_IDENTITY_UNAVAILABLE: PID ${record.pid} is alive, but exact start-time identity is unavailable for an external process; recovery fails closed`, 3);
        }
    ...
    try {
        ...
        if (failure)
            throw new AggregateError([failure, cleanupError], "Lock recovery failed and its owned recovery claim could not be removed");
        throw cleanupError;
    }
    if (failure)
        throw failure;
