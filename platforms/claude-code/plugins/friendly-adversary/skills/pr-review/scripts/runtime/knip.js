...
export function knipPrRelevant(files, diff) {
    if (files.some(file => /(?:^|\/)(?:package\.json|(?:pnpm-workspace|pnpm-lock)\.yaml|(?:package-lock|npm-shrinkwrap)\.json|yarn\.lock|bun\.lockb?|(?:tsconfig|jsconfig)[^/]*\.json|(?:knip|vite|next|webpack|rollup|astro|nuxt|svelte|vitest|jest|nx)[^/]*\.(?:jsonc?|[cm]?[jt]s))$/i.test(file)))
        return true;
    ...
    for (const line of diff.split('\n')) {
        ...
        if (/^\s*(?:import|export)\s+(?:type\s+)?\{/.test(text))
            moduleList = true;
        if (moduleList && /^[+-]/.test(line))
            return true;
        if (moduleList && /\}/.test(text))
            moduleList = false;
    }
...
export async function knipApplicable(repo, files, base, audit) {
    ...
    try {
        if (!(await lstat(path.join(repo, 'package.json'))).isFile())
            return false;
    }
    catch (error) {
...
export async function knipProjectRoots(repo, files, base, audit) {
    if (await knipApplicable(repo, files, base, audit))
        return [repo];
    ...
    for (const candidate of [...candidates].sort()) {
        ...
        if (await knipApplicable(candidate, scoped, base, audit))
            roots.push(candidate);
    }
    return roots;
}
export async function validateKnipOutput(directory) {
    try {
        const report = JSON.parse(await readFile(path.join(directory, 'stdout.json'), 'utf8'));
        if (!Array.isArray(report.issues))
            return 'Knip did not produce its expected JSON issue report';
        const coverage = JSON.parse(await readFile(path.join(directory, 'coverage.json'), 'utf8'));
        if (!Array.isArray(coverage.configurationHints))
            return 'Knip coverage metadata is missing configuration hints';
        if (coverage.configurationHints.length)
            return 'Knip reported configuration hints; inspect coverage.json before trusting unused-code findings';
    }
    catch {
        return 'Knip report or coverage metadata is missing or invalid';
    }
    return undefined;
}
