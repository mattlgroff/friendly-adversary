...
import { createReviewAuthority, ensureAuthorityRoot, liveReviewAuthorityId, revokeReviewAuthorityById, RUN_PLAN_FILE, } from "./authority.js";
...
import { ensureSafeOutputRoot, readJson, renameWithWindowsRetry, sha256, writeFileAtomic, writeJsonAtomic } from "./fs-utils.js";
...
import { localCodexRuntime, runCodexLenses } from "./lens-runner.js";
...
function languages(files) {
    const found = [];
    if (files.some((file) => TYPESCRIPT_EXTENSIONS.has(path.extname(file).toLowerCase())))
        found.push("typescript");
    if (files.some((file) => PYTHON_EXTENSIONS.has(path.extname(file).toLowerCase())))
        found.push("python");
    return found;
}
function runPrefix(headSha) {
    return `${new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-")}-${headSha.slice(0, 10)}-`;
}
async function privateRunsRoot() {
    ...
    if (metadata.isSymbolicLink() || !metadata.isDirectory())
        throw new FriendlyAdversaryError("Private PR review state is redirected or not a directory", 3);
    await chmod(root, 0o700);
    const canonical = await realpath(root);
    if (path.dirname(canonical) !== authorityState)
        throw new FriendlyAdversaryError("Private PR review state escaped its authority root", 3);
    return canonical;
}
async function materializeLensDefinitions(ids, runDirectory) {
    ...
    for (const id of ids) {
        ...
        if (!source)
            throw new FriendlyAdversaryError(`Installed lens definition is missing: ${id}`, 2);
    ...
    if (!findingContract)
        throw new FriendlyAdversaryError("Installed finding contract is missing", 2);
...
async function validateRun(runDirectory) {
    ...
    const receipt = await readJson(path.join(runReal, RECEIPT_FILE)).catch(() => {
    ...
    if (repoReal !== receipt.repositoryRoot)
        throw new FriendlyAdversaryError("The repository path is no longer canonical", 2);
...
function derivedIncompleteReasons(receipt) {
...
export async function runReview(options, hooks = {}) {
    const expectedLenses = [...new Set(options.expectedLenses)].sort();
    if (!expectedLenses.length)
        throw new FriendlyAdversaryError("At least one lens is required", 2);
    const git = await inspectGit(options.repo, options.base);
    if (!git.changedFiles.length)
        throw new FriendlyAdversaryError("No changes were found against the selected base.", 2);
    ...
    try {
        ...
        receipt.toolRuns = await collectTools({
            ...
            afterRepositoryTool: async () => {
                ...
                if (snapshot.headSha !== git.headSha || snapshot.diffHash !== git.diffHash)
                    repositoryChangedDuringChecks = true;
            },
        });
        const after = await inspectGit(git.repositoryRoot, git.baseSha);
        if (repositoryChangedDuringChecks || after.headSha !== git.headSha || after.diffHash !== git.diffHash)
            receipt.incompleteReasons.push("A repository check changed the reviewed snapshot");
        ...
        const removeFailedPath = hooks.removeFailedPath ?? ((target) => rm(target, { recursive: true, force: true }));
        ...
        if (cleanupErrors.length) {
            throw new AggregateError([error, ...cleanupErrors], "Review collection failed and private-state cleanup was incomplete");
        }
        throw error;
    }
}
export async function runReviewWithLenses(options, runtime = localCodexRuntime) {
...
function readModelMetadata(content, label) {
    const get = (field) => {
        const value = new RegExp(`^- ${field}:\\s*(.+?)\\s*$`, "mu").exec(content)?.[1]?.trim();
        if (!value || value.length > 200)
            throw new FriendlyAdversaryError(`${label} must record '- ${field}: <value>'`, 2);
        return value;
    };
    return { model: get("Model"), effort: get("Effort"), host: get("Host") };
}
async function requireCompletedDocuments(runReal, expectedLenses) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected))
        throw new FriendlyAdversaryError("Lens reports do not match the expected set", 2);
    const lenses = [];
    for (const file of actual) {
        const content = await readFile(path.join(runReal, "lenses", file), "utf8");
        if (!content.trimEnd().endsWith(LENS_COMPLETE))
            throw new FriendlyAdversaryError(`${file} is incomplete`, 2);
    ...
    if (!adjudication.trimEnd().endsWith(DOCUMENT_COMPLETE) || !report.trimEnd().endsWith(DOCUMENT_COMPLETE))
        throw new FriendlyAdversaryError("Final review documents are incomplete", 2);
    for (const [file, content] of [["adjudication.md", adjudication], ["report.md", report]]) {
        const secret = detectRecognizableSecret(content);
        if (secret)
            throw new FriendlyAdversaryError(`${file} contains a recognizable ${secret} value`, 2);
    }
...
async function walk(root, current = root) {
    ...
    for (const entry of await readdir(current, { withFileTypes: true })) {
        ...
        if (metadata.isSymbolicLink())
            throw new FriendlyAdversaryError(`Artifact tree contains a symbolic link: ${absolute}`, 2);
        if (metadata.isDirectory())
            files.push(...await walk(root, absolute));
        else if (metadata.isFile())
            files.push(path.relative(root, absolute).split(path.sep).join("/"));
        else
            throw new FriendlyAdversaryError(`Artifact tree contains an unsupported entry: ${absolute}`, 2);
    }
    return files.sort();
}
...
async function copyArtifactTree(source, destination) {
    await mkdir(destination, { recursive: true, mode: 0o700 });
    for (const relative of await walk(source)) {
        if (relative === RUN_PLAN_FILE)
            continue;
...
async function terminalClaim(runReal) {
    return readFile(path.join(`${runReal}.scratch`, "terminal-operation"), "utf8").catch((error) => {
        if (error.code === "ENOENT")
            return undefined;
        throw error;
    });
}
async function claimTerminalOperation(runReal, claimValue) {
...
async function recoverPublishedReview(finalDirectory, runReal) {
...
async function sealReviewAttempt(runReal, receipt, hooks) {
    ...
    if (finalDirectory !== publication)
        throw new FriendlyAdversaryError("The final review publication path is unavailable", 3);
    const ensurePublicationRoot = async () => {
        const safeOutput = await ensureSafeOutputRoot(receipt.repositoryRoot, outputRoot);
        if (path.join(safeOutput, RUNS_DIRECTORY) !== runsRoot)
            throw new FriendlyAdversaryError("The final review publication path is unavailable", 3);
        await mkdir(runsRoot, { recursive: true, mode: 0o700 });
    };
    const claimedStaging = (claim) => {
    ...
    const finishClaimedSeal = async (staging) => {
        try {
    ...
    if (current.headSha !== receipt.git.headSha || current.diffHash !== receipt.git.diffHash)
        throw new FriendlyAdversaryError("The reviewed Git snapshot changed before publication", 3);
    ...
    if (receipt.incompleteReasons.length) {
        ...
        if (!report.includes(INCOMPLETE_STATUS))
            throw new FriendlyAdversaryError("Incomplete review report must disclose its coverage gaps", 2);
    }
    await writeHtmlCompanion(runReal, "report.md", "report.html", "Friendly Adversary PR review");
    await ensurePublicationRoot();
    const finalEntry = await lstat(finalDirectory).catch((error) => {
        if (error.code === "ENOENT")
            return undefined;
        throw error;
    });
    if (finalEntry) {
        if (finalEntry.isSymbolicLink() || !finalEntry.isDirectory())
            throw new FriendlyAdversaryError("The final review publication path is redirected or not a directory", 3);
        await verifyReview(finalDirectory);
    ...
    if (authorityId)
        await revokeReviewAuthorityById(authorityId);
    ...
    try {
        ...
        for (const file of files)
            lines.push(manifestLine(file, await readFile(path.join(staging, ...file.split("/")))));
        ...
        if (finalSnapshot.headSha !== receipt.git.headSha || finalSnapshot.diffHash !== receipt.git.diffHash)
            throw new FriendlyAdversaryError("The reviewed Git snapshot changed during publication", 3);
...
export async function sealReview(runDirectory, hooks = {}) {
    const { runReal, receipt } = await validateRun(runDirectory);
    if (receipt.status !== "collected" && receipt.status !== "incomplete")
        throw new FriendlyAdversaryError(`Cannot seal a run in status ${receipt.status}`, 2);
    try {
...
async function readArtifactManifest(runReal) {
    ...
    for (const line of (await readFile(path.join(runReal, "artifacts.sha256"), "utf8")).trimEnd().split("\n")) {
        ...
        if (!match)
            throw new FriendlyAdversaryError("Artifact manifest is malformed", 2);
        expected.set(JSON.parse(match[3]), { hash: match[1], size: Number(match[2]) });
    }
    return expected;
}
async function verifyArtifactTree(runReal) {
    ...
    if (actual.length !== expected.size || actual.some((file) => !expected.has(file)))
        throw new FriendlyAdversaryError("Artifact set differs from the seal", 3);
    for (const file of actual) {
        const content = await readFile(path.join(runReal, ...file.split("/")));
        const record = expected.get(file);
        if (record.size !== content.byteLength || record.hash !== sha256(content))
            throw new FriendlyAdversaryError(`Artifact verification failed: ${file}`, 3);
    }
    return { valid: true, artifacts: actual.length + 1 };
}
export async function verifyReview(runDirectory) {
    const { runReal, receipt } = await validateRun(runDirectory);
    if (receipt.status !== "sealed" && receipt.status !== "sealed-incomplete")
        throw new FriendlyAdversaryError(`Run is not sealed: ${receipt.status}`, 2);
    return verifyArtifactTree(runReal);
}
export async function statRun(runDirectory) {
    const { runReal, receipt } = await validateRun(runDirectory);
    let artifactBytes = 0;
    for (const file of await walk(runReal))
        artifactBytes += (await stat(path.join(runReal, ...file.split("/")))).size;
